...
- **Auditoria de cobertura do catálogo** (pedido numa conversa — "existem novas pages e funcionalidades que ainda não estão lá"): o sistema de permissão tem **três listas** que precisam andar juntas, e nada as obrigava a isso, então dessincronizaram silenciosamente conforme telas novas foram entrando — (1) `CATALOGO` (`lib/itens-permissao.js`), única fonte de páginas de um perfil CUSTOMIZADO; (2) `PAGINAS_DE_TRABALHO`/`ABAS_CONFIG_ADMIN` (`lib/perfis.js`), lista hardcoded do que os 6 perfis FIXOS enxergam; (3) os `data-page` reais nos partials, que o front usa pra esconder/mostrar. Dois bugs achados e corrigidos:
  - **Faltavam no catálogo** (existiam em 2 e 3): `tracos-descartados`, `one-page-report`, `config-paletes` e `config-notificacoes`. Como `paginasPermitidas()` (`lib/perfis-customizados.js`) deriva a lista **só** do catálogo, nenhum perfil customizado conseguia ver essas telas — e não havia como liberar, já que nem apareciam no formulário de "+ Criar novo tipo de perfil" pra serem marcadas. Perfis fixos nunca foram afetados.
  - **Faltavam nos perfis fixos** (existiam em 1 e 3): `qualidade-tracos` (CEP), `consulta-tracos` e `tv` (Modo TV). Como `_aplicarVisibilidadeDoMenu` (`public/js/app-core.js`) esconde todo `data-page` fora da lista e `showPage()` recusa navegar até ele, essas 3 telas ficavam invisíveis pra **todo perfil cadastrado** — inclusive "Administrativo"; só o Administrador MASTER (que ignora a lista inteira) enxergava. Contrariava o "modelo novo" declarado no próprio arquivo ("TODAS são visíveis a TODOS os perfis").

  Os padrões por perfil fixo saem automaticamente de `permissoesPadraoDoPerfilFixo()` (Traços Descartados é área `injetora` → quem edita a área começa em "Acesso Total", o resto em "Apenas Visualizar"; One Page Report não tem área → "Apenas Visualizar"; as 2 abas de Configurações → só "Administrativo"). `test/catalogo-permissoes-cobertura.test.js` trava as três listas em sincronia (mais invariantes do catálogo: sem id duplicado, todo `pai` existente, toda `area` real) — os testes falham em 7 pontos se rodados contra o código anterior à correção, então esta classe de bug não volta despercebida.
...
- **Análise Focada** (`Alt+F`) e **Consulta de Insumos por Traço** (`Alt+I`) — **decisão revista numa conversa seguinte**: inicialmente ficaram sem `Alt+dígito` por parecerem só páginas de "detalhe" chegadas por drill-down; percebeu-se depois que as 2 também têm botão próprio no dropdown "Traços" da barra de navegação (`nav-tabbar.html`), não são só destino de `Ctrl+clique` — por consistência com as demais páginas do mesmo dropdown, ganharam navegação direta também. O `Ctrl+clique` continua funcionando do mesmo jeito, como atalho secundário a partir de Registro de Baterias/Relatório de Injeção.
- **Manutenção** — a auditoria achou um atalho REAL já em produção que nunca tinha sido catalogado — `Ctrl + passar o mouse` numa linha da tabela mostra um preview flutuante da trajetória (`_mostrarPreviewTrajetoria`, `public/js/manutencao.js`), sem abrir o modal completo. Comportamento não mudou, só passou a aparecer em `REFERENCIA_CONFIG` (documentado, descobrível). **Correção numa conversa seguinte** ("o setor de manutenção ficou sem atalho"): documentar o Ctrl+hover não é o mesmo que dar navegação até a página — Manutenção continuou sem `Alt+dígito` próprio, diferente da irmã dela (Setor de Qualidade, `Alt+Q`). Ganhou `Alt+N`. O teste de auditoria foi reforçado (`toda página com botão de navegação em nav-tabbar.html tem um Alt+dígito`) — comprovadamente pega esta classe de bug (falha se rodado contra o código anterior a esta correção).
...
**Importante em produção atrás de proxy reverso** (Caddy, ver `deploy/instalar-https.sh`): esse IP precisa vir de `ipRealDoRequest()` (`lib/ip-cliente.js`), nunca de `req.socket.remoteAddress` direto — atrás de um reverse proxy, o socket que o Node enxerga é sempre o do PRÓPRIO Caddy (localhost), igual pra todo mundo. Usar o IP errado aqui não é só "a autocura não funciona": ela ativamente RELIGA a entrada de um dispositivo já autorizado pro `deviceId` de outro que reconectou depois (já que todos bateriam no mesmo IP de proxy), roubando a autorização silenciosamente — foi um bug real em produção, relatado como "o deviceId muda, temos que ficar reautorizando de vez em quando". `ipRealDoRequest()` lê o header `X-Forwarded-For` que o Caddy já agrega por padrão em todo `reverse_proxy` (nenhuma configuração extra necessária no Caddyfile), pegando sempre o ÚLTIMO valor da lista (o mais próximo do proxy confiável — um cliente malicioso pode forjar seu próprio `X-Forwarded-For`, mas não consegue impedir o Caddy de acrescentar o IP real dele no fim da lista). Sem proxy nenhum na frente (dev local), cai no `socket.remoteAddress` de sempre. Coberto por `test/ip-real-atras-proxy.test.js`.

Chegou a existir também uma terceira via de reconhecimento por **certificado de cliente TLS (mTLS)** — removida depois: TLS 1.3 exige assinatura RSA-PSS pra autenticação de cliente, e boa parte das pilhas TLS mais antigas do Windows (Schannel/.NET clássico, usado por PowerShell 5.1 e por extensão o próprio Chrome/Edge nesses sistemas) não sabe assinar com RSA-PSS pra isso — o navegador simplesmente nunca oferece o certificado (nem mostra o seletor de qual usar), silenciosamente, sem solução viável sem rebaixar a versão do TLS pro servidor inteiro. A correção real pro atrito que motivou aquela tentativa era a de `ipRealDoRequest()` acima.
...
| **Backups Automáticos** | Lista os backups diários gerados pelo servidor (ver abaixo — escopo Geral, dados de produção + config/usuários), com link de download pra cada um. |
...
O próprio `server.js` gera um backup **Geral** todo fim de dia, sem depender de ninguém com o navegador aberto (pedido registrado numa conversa — "eu quero que seja um backup geral"; até então era só o escopo "Dados", produção da fábrica sem config/usuários — ver `test/backup-automatico-escopo-geral.test.js`):
...
- Arquivos ficam em `backups-automaticos/` (fora de `public/`, nunca servida como arquivo estático comum), nomeados por data: `backup-dados_AAAA-MM-DD.zip` — nome mantido por compatibilidade com a rotação/download já existentes, mesmo o conteúdo agora sendo o escopo Geral (dados de produção + `config.json`/`security.json`/`usuarios.json`/`operadores.json`).
...
- **Sincronizar com um backup manual (conversa anterior que motivou a mudança)**: até aqui, backup manual (Dados/Geral) e o job automático diário eram 100% independentes — baixar um backup manual nunca afetava `backups-automaticos/`. Agora, depois de qualquer backup manual bem-sucedido, o front pergunta (`LW.mostrarConfirmacao`) se a pessoa quer que ESSE backup também vire o automático de hoje — nunca acontece sozinho. Se ela confirmar, `POST /sincronizar-backup-automatico` (`lib/rotas/backup.js`, `{ tipo: 'dados'|'geral' }`) sobrescreve `backup-dados_<hoje>.zip` com o MESMO conteúdo do manual (mesmo nome de arquivo — não cria um segundo) e reinicia a contagem de retenção de 3 dias a partir de agora; como o job agendado já pula o dia se o arquivo existir, ele não roda de novo mais tarde sobrescrevendo o que foi sincronizado manualmente. Continua permitindo escolher `'dados'` ou `'geral'` nessa rota manual — só o job **agendado** (sem intervenção nenhuma) é que ficou fixo em Geral. Coberto por `test/backup-sincronizar-automatico.test.js`.
