...
### Fase 5 (plano) — Progresso REAL na fase `imprimindo`, sem estimativa
...
| 5.4 | Aposentar a estimativa por média móvel entre jobs (`_msPorOperacaoImpressao`/`_iniciarTickerImpressao`/`_registrarDuracaoImpressao`) — não faz mais falta nessa fase, já que agora o progresso é medido de verdade a cada página; `segundosRestantes` passa a vir do tempo médio JÁ OBSERVADO nas páginas anteriores DESTE MESMO job (mais preciso ainda: reflete a complexidade real da operação sendo impressa agora, não uma média de jobs antigos possivelmente bem diferentes) | `lib/rotas/exportar-pdf.js` | Baixo — é remoção de código + um cálculo local mais simples no lugar |
| 5.5 | Cliente (`_progressoServidor`, `public/js/analise-focada.js`) continua recebendo `fase:'imprimindo', feito, total, segundosRestantes` por SSE — mesmo formato de evento — mas `feito`/`total` agora são contagem real de páginas (ex.: "3 de 12"), não mais uma % 0-95; ajustar o cálculo da barra (92%-100%) e o texto pra usar `feito/total` como fração normal, igual já faz na fase `ajustando`, em vez da lógica atual de "`feito` já vem em porcentagem" | `public/js/analise-focada.js` (`_progressoServidor`) | Baixo — muda só a leitura de um número que já chega por SSE, layout da barra já existe |
...
**Status:** plano — ainda não implementado.
