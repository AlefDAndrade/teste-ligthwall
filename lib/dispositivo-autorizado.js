...
//
// NOTA: chegou a existir aqui uma camada extra de reconhecimento via
// certificado de cliente (mTLS) — removida (ver histórico do git se precisar
// resgatar) por incompatibilidade prática: navegadores/Windows em TLS 1.3
// exigem assinatura RSA-PSS pra autenticação de cliente, e boa parte das
// pilhas TLS mais antigas do Windows (Schannel/.NET clássico usado por
// PowerShell 5.1, e por extensão o próprio Chrome/Edge nesses sistemas) não
// sabe assinar com RSA-PSS pra isso — o navegador simplesmente nunca oferece
// o certificado (nem mostra o seletor), silenciosamente. Sem solução viável
// sem rebaixar a versão do TLS pra todo o servidor. A correção de verdade
// pro atrito que motivou aquilo (dispositivo "perdendo" autorização com
// frequência) era outra: ver ipRealDoRequest, abaixo.
...
module.exports = function criarDispositivoAutorizado({ fs, path, DB_DIR, sessao, sessaoUsuario, perfis, podeEditarArea }) {
  ...
  //
  // IMPORTANTE: `ip` precisa vir de ipRealDoRequest() (lib/ip-cliente.js),
  // nunca de req.socket.remoteAddress direto — atrás do Caddy (reverse
  // proxy, ver deploy/instalar-https.sh), o socket é sempre o do PRÓPRIO
  // Caddy (localhost), igual pra todo mundo. Usar o IP errado aqui não é só
  // "a autocura não funciona": ela ativamente RELIGA a entrada de um
  // dispositivo já autorizado pro deviceId de outro que reconectou depois
  // (já que todos bateriam no mesmo IP de proxy), roubando a autorização
  // silenciosamente — foi exatamente o bug relatado como "o deviceId muda,
  // temos que ficar reautorizando de vez em quando".
  ...
  function negarControleDeOperacao(res, req, deviceId) {
    if (!dispositivoAutorizado(deviceId, null)) {
