...
module.exports = function criarDispositivoAutorizado({ fs, path, DB_DIR, sessao, sessaoUsuario, perfis, podeEditarArea, certificadoDispositivo }) {
  ...
  // Usado pelo novo endpoint público GET /status-dispositivo (ver
  // lib/rotas/status-dispositivo.js) — o front-end (public/js/data.js,
  // _esteDispositivoEstaNaLista()) só conhecia a lista por deviceId/cookie
  // até aqui; certificado (mTLS) nunca tinha um jeito do NAVEGADOR saber que
  // valia pra ele, só o servidor sabia (em podeControlarOperacao). Resultado:
  // um dispositivo autorizado SÓ por certificado via a tela travada com a
  // mensagem "dispositivo não autorizado" mesmo o servidor aceitando a
  // requisição de verdade — o JS nunca chegava a mandar, travava antes.
  // deviceId/ip null de propósito: queremos checar só a via do certificado
  // aqui, sem misturar com a lógica de religar por IP (essa continua restrita
  // a quando alguém de fato manda um deviceId, dentro de dispositivoAutorizado
  // acima).
  function certificadoAutorizadoNesteRequest(req) {
    return dispositivoAutorizado(null, null, certSerialDoRequest(req));
  }
  ...
  return {
    ...
    certificadoAutorizadoNesteRequest,
  };
};
