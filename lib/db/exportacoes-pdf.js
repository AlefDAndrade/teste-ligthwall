...
module.exports = function criarDbExportacoesPdf(db) {
  ...
  function criarRegistroExportacaoPdf({ jobId, nomeArquivo, criadoEm }) {
    db.prepare(`
      INSERT INTO exportacoes_pdf (job_id, nome_arquivo, status, criado_em)
      VALUES (?, ?, 'processando', ?)
    `).run(jobId, nomeArquivo, criadoEm);
  }
