...
module.exports = function criarDbTracosDescartados(db) {
  ...
  /**
   * Edita 1 traço descartado já existente — SUBSTITUI por completo os
   * campos editáveis (mesmo padrão de /salvar-parada, lib/rotas/paradas.js:
   * um PUT lógico, não um PATCH parcial). `id`/`registrado_em` NUNCA mudam
   * aqui (o momento do registro original é auditoria, não deveria se
   * mexer só porque alguém corrigiu um valor depois) — só os campos de
   * dado (insumos/motivo/turno/data/operador) são atualizáveis. Devolve
   * `true` se algo foi de fato encontrado e atualizado (mesmo contrato de
   * excluirTracoDescartado, abaixo).
   */
  function editarTracoDescartado(id, campos) {
    const atual = db.prepare('SELECT * FROM tracos_descartados WHERE id = ?').get(id);
    if (!atual) return false;
    const mesclado = tracoDescartadoParaRow({
      ...rowParaTracoDescartado(atual),
      ...campos,
      id: atual.id,
      registrado_em: atual.registrado_em,
    });
    db.prepare(`
      UPDATE tracos_descartados SET
        data = @data, turno = @turno, cimento = @cimento, agua = @agua, eps = @eps,
        superplast = @superplast, incorporador = @incorporador, tempo_batida = @tempo_batida,
        motivo = @motivo, operador_nome = @operador_nome
      WHERE id = @id
    `).run(mesclado);
    return true;
  }
  ...
  /**
   * Exclui 1 traço descartado pelo id — mesmo contrato de
   * excluirCargaExpedicao/excluirOcorrenciaSeguranca (devolve `true` só se
   * algo foi de fato apagado, pra rota devolver "não encontrado" com
   * segurança em vez de responder OK pra um id que não existia).
   */
  function excluirTracoDescartado(id) {
    const resultado = db.prepare('DELETE FROM tracos_descartados WHERE id = ?').run(id);
  ...
  return {
    ...
    editarTracoDescartado,
    excluirTracoDescartado,
