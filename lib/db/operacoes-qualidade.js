...
module.exports = function criarOperacoesQualidade(db) {
  ...
  function operacaoParaRow(r) {
    return {
      ...
      // ausente em toda operação registrada ao vivo (o caminho de sempre),
      // então cai no default seguro (0/null) pra não quebrar nenhum outro
      // ponto que já chama operacaoParaRow (registro ao vivo, importação,
      // restauração de backup — nenhum desses manda estes 3 campos).
      origem_offline: r.origem_offline === true || r.origem_offline === 1 ? 1 : 0,
      validado_por: r.validado_por ?? null,
      validado_em: r.validado_em ?? null,
    };
  }
  ...
  function rowParaOperacao(row) {
    return {
      ...
      // Auditoria de Registro Offline — ver comentário em operacaoParaRow, acima.
      origem_offline: !!row.origem_offline,
      validado_por: row.validado_por || null,
  ...
  const SQL_INSERIR_OPERACAO = `
    INSERT INTO operacoes (
      ...
      tempo_min, qtd_tracos, houve_atraso, motivo_atraso, tipo_montagem,
      ...
      tracos_json, avaliado, modo_teste, operador_nome, origem_offline, validado_por, validado_em, criado_em
    ) VALUES (
      @id, @data, @turno, @dimensao, @capacidade, @id_bateria, @inicio, @fim, @desemplaque,
      @tempo_min, @qtd_tracos, @houve_atraso, @motivo_atraso, @tipo_montagem,
      @bercos_personalizados, @bercos_dimensoes, @total_paineis, @m2_total, @placas_cimenticia,
  ...
  function relatorioBercos() {
    const rows = db.prepare(`
      SELECT o.id AS id_operacao, o.data, o.turno, o.id_bateria, o.tipo_montagem,
             o.capacidade, o.houve_atraso, o.motivo_atraso,
  ...
  function detalheOperacao(idOperacao) {
    ...
    const tracos = usos.map(u => {
      const ajustes = db.prepare(
        'SELECT id, ordem, tempo_batida, cimento, agua, eps, superplast, incorporador, registrado_em FROM ajustes WHERE id_traco = ? ORDER BY ordem ASC'
      ).all(u.id_traco);
      // Insumos CUSTOM (Fase 6, ver PLANO-insumos-dinamicos-receitas.md) —
      // mesma fonte (traco_insumos/ajuste_insumos, Fase 1/2), mas neste
      // endpoint o formato de sempre é "achatado" (original: {campo:
      // valor}, cada item de ajustes[] com o campo direto — ver
      // original/ajustes logo abaixo), então segue o mesmo padrão em vez
      // do {original,ajustes} aninhado que rowParaTraco usa.
      // NOMES_INSUMOS_PADRAO filtra fora qualquer linha "fantasma" que a
      // migração da Fase 1 tenha deixado pros 5 Padrão (ver comentário
      // em migrarInsumosFixosParaDinamico, lib/db/tracos.js) — aqui os 5
      // Padrão só vêm das colunas fixas acima, nunca desta tabela.
      const insumosCustomOriginaisRows = db.prepare('SELECT insumo, valor FROM traco_insumos WHERE id_traco = ?').all(u.id_traco)
        .filter(r => !db.NOMES_INSUMOS_PADRAO.has(r.insumo));
      const insumosCustomOriginais = Object.fromEntries(insumosCustomOriginaisRows.map(r => [r.insumo, r.valor]));
      const idsAjuste = ajustes.map(a => a.id);
      const insumosCustomPorAjuste = idsAjuste.length
        ? db.prepare(`SELECT id_ajuste, insumo, valor FROM ajuste_insumos WHERE id_ajuste IN (${idsAjuste.map(() => '?').join(',')})`)
            .all(...idsAjuste).filter(r => !db.NOMES_INSUMOS_PADRAO.has(r.insumo))
        : [];
      ajustes.forEach(a => {
        const doAjuste = insumosCustomPorAjuste.filter(r => r.id_ajuste === a.id);
        if (doAjuste.length) a.insumos_custom = Object.fromEntries(doAjuste.map(r => [r.insumo, r.valor]));
        delete a.id; // não fazia parte do formato de saída original, era só chave interna pra achar os ajuste_insumos acima
      });
      // BUG CORRIGIDO (relatado: "aparece nos ajustes, mas não na receita")
      // — igual aos 5 Padrão (que sempre mostram o campo mesmo com
      // original NULL, contanto que tenha algum ajuste — ver
      // colapsarOriginalEAjustes, lib/db/tracos.js), um insumo Custom que
      // só tem valor via AJUSTE (nunca preencheu o campo principal —
      // possível: no formulário, "+" exige preencher, mas quem preenche é
      // o total exibido, que pode vir só do ajuste, ver insumoPreenchido
      // em operacao.js) também precisa aparecer em insumos_custom, com
      // original ausente. Union de nomes de traco_insumos E ajuste_insumos
      // — antes só olhava traco_insumos, então esse caso sumia da Receita
      // Utilizada mesmo tendo ajuste visível ali do lado.
      const nomesCustomUniao = new Set([
        ...insumosCustomOriginaisRows.map(r => r.insumo),
        ...insumosCustomPorAjuste.map(r => r.insumo),
      ]);
      const insumosCustom = {};
      nomesCustomUniao.forEach(nome => { insumosCustom[nome] = insumosCustomOriginais[nome] ?? null; });
      // Lista COMPLETA das remedições de densidade/flow (não só a última) —
      // igual ao que o Relatório de Injeção mostra no painel de
      // detalhamento (ver _construirDetalheRelatorio, dashboard.js). O
      // valor final de cada campo (densidade/flow, abaixo) continua sendo
      // só a última leitura (ou o original, se nunca foi remedido).
      const leiturasDensidade = db.prepare(
        "SELECT valor FROM leituras_resultado WHERE id_traco=? AND campo='densidade' ORDER BY ordem ASC"
      ).all(u.id_traco).map(r => r.valor);
      const leiturasFlow = db.prepare(
        "SELECT valor FROM leituras_resultado WHERE id_traco=? AND campo='flow' ORDER BY ordem ASC"
      ).all(u.id_traco).map(r => r.valor);
      return {
        id_traco: u.id_traco,
        ...
        original: {
          ...
          densidade: u.densidade_original, flow: u.flow_original,
        },
        // Só entra a chave se o traço de fato tiver algum Custom (em
        // qualquer uma das 2 fontes, união acima) — não polui a saída de
        // quem nunca usou a feature (mesmo critério de montarInsumosCustom,
        // lib/db/tracos.js).
        ...(Object.keys(insumosCustom).length ? { insumos_custom: insumosCustom } : {}),
        densidade: leiturasDensidade.length ? leiturasDensidade[leiturasDensidade.length - 1] : u.densidade_original,
        flow: leiturasFlow.length ? leiturasFlow[leiturasFlow.length - 1] : u.flow_original,
        densidade_leituras: leiturasDensidade,
        flow_leituras: leiturasFlow,
