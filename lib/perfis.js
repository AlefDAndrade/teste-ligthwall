...
const PAGINAS_DE_TRABALHO = [
  ...
  'oee', 'metas', 'paradas', 'tracos-descartados',
  'analise-focada', 'analise-bercos', 'analise-operacional',
  'setor-qualidade', 'manutencao', 'one-page-report',
  // ─── Faltavam aqui (achado na mesma auditoria do catálogo de itens,
  // ver lib/itens-permissao.js): as 3 telas abaixo têm `data-page` no
  // menu/nav, então _aplicarVisibilidadeDoMenu (public/js/app-core.js)
  // escondia todas elas de TODO perfil cadastrado — incluindo
  // "Administrativo" —, e showPage() recusava navegar até lá. Só o
  // Administrador MASTER (senha mestra, que ignora esta lista inteira)
  // enxergava. Não era intencional: contraria o "modelo novo" declarado
  // logo acima ("TODAS são visíveis a TODOS os perfis") e as 3 já
  // constavam normalmente no catálogo de perfis customizados.
  'qualidade-tracos',   // CEP / Dashboard de Traço
  'consulta-tracos',    // Consulta de Insumos por Traço (auxiliar do CEP)
  'tv',                 // Modo TV (abre tv.html em outra aba)
];
...
const ABAS_CONFIG_ADMIN = [
  ...
  'config-automacao', 'config-sql', 'config-autorizados',
  'config-notificacoes', 'config-paradas', 'config-tipos-manutencao', 'config-prioridades',
  // Registro de Operação Offline (PWA) — itens 6/7 do plano, ver README.
  'config-operacoes-offline',
];
...
function permissoesPadraoDoPerfilFixo(perfilId) {
  ...
  for (const item of itensPermissao.CATALOGO) {
    ...
    } else if (item.id === itensPermissao.ITEM_NOTIFICACAO_PEDIDO_PECA) {
      // Outro caso especial (ver comentário no catálogo,
      // lib/itens-permissao.js): quem RECEBE por padrão é exatamente
      // quem PODE ACEITAR um pedido de peça (ver podeAceitarPedidoPeca,
      // server.js) — Supervisão, Encarregado ou Administrador; perfil
      // Manutenção não recebe (é quem ABRE o pedido, não quem aceita).
      mapa[item.id] = (perfilId === 'Supervisao' || perfilId === 'Encarregado' || ehPerfilDeAdmin(perfilId))
        ? 'total' : 'ocultar';
    } else if (item.id === itensPermissao.ITEM_NOTIFICACAO_PECA_RECEBIDA) {
      // Outro caso especial (ver comentário no catálogo,
      // lib/itens-permissao.js): quem RECEBE por padrão é Manutenção,
      // Supervisão, Encarregado ou Administrador — pedido do usuário
      // (diferente do item de Pedido de Peça, acima: aqui Manutenção
      // TAMBÉM recebe, porque é quem dá seguimento ao conserto assim que
      // a peça chega).
      mapa[item.id] = (perfilId === 'Manutencao' || perfilId === 'Supervisao' || perfilId === 'Encarregado' || ehPerfilDeAdmin(perfilId))
        ? 'total' : 'ocultar';
    } else if (item.id === itensPermissao.ITEM_NOTIFICACAO_MANUTENCAO_PROGRAMADA) {
      // Outro caso especial (ver comentário no catálogo,
      // lib/itens-permissao.js) — pedido do usuário: quando uma
      // manutenção PROGRAMADA é agendada, TODOS os perfis devem ser
      // notificados (diferente dos 3 itens de notificação acima, que
      // restringem por perfil) — todo perfil fixo começa 'total' aqui;
      // continua ajustável depois pela mesma tela de Configurações →
      // Notificações, caso alguém queira restringir pra um perfil
      // específico.
      mapa[item.id] = 'total';
    } else if (item.id === itensPermissao.ITEM_NOTIFICACAO_MANUTENCAO_PROGRAMADA_LEMBRETE) {
      // Outro caso especial (ver comentário no catálogo,
      // lib/itens-permissao.js) — mesmo padrão do item de cima
      // (agendamento criado): todo perfil fixo começa 'total' aqui;
      // continua ajustável depois pela mesma tela de Configurações →
      // Notificações, caso alguém queira restringir pra um perfil
      // específico.
      mapa[item.id] = 'total';
    } else if (item.id === itensPermissao.ITEM_NOTIFICACAO_ACEITE_CHAMADO) {
      // Outro caso especial (ver comentário no catálogo,
      // lib/itens-permissao.js): quem RECEBE por padrão é Supervisão,
      // Encarregado ou Administrador — mesmo grupo do item de Pedido de
      // Peça, acima: são os perfis que acompanham o andamento do
      // chamado, não Manutenção (que é quem dispara a ação de aceitar,
      // já sabe que aceitou).
      mapa[item.id] = (perfilId === 'Supervisao' || perfilId === 'Encarregado' || ehPerfilDeAdmin(perfilId))
        ? 'total' : 'ocultar';
    } else if (item.id === itensPermissao.ITEM_NOTIFICACAO_ATUALIZACAO_ETIQUETA) {
      // Outro caso especial (ver comentário no catálogo,
      // lib/itens-permissao.js): notificação GENÉRICA de "qualquer
      // atualização numa etiqueta" — mesmo padrão do item de Abertura de
      // Chamado, no topo deste bloco: o padrão de cada perfil FIXO é
      // "esse perfil mexe com Manutenção?" (mesma checagem de
      // 'manutencao-chamado'/'manutencao-execucao'), já que é um aviso
      // amplo pra quem acompanha o setor no geral, não restrito a um
      // grupo específico de aprovadores como Pedido de Peça/Aceite.
      mapa[item.id] = podeEditar(perfilId, 'manutencao') ? 'total' : 'ocultar';
