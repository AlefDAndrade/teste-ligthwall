...
module.exports = function criarNotificacoesPush({
  ...
  // Mesma ideia, pro item "Notificar Aceite de Chamado" (ver catálogo,
  // lib/itens-permissao.js, e notificarAceiteChamado, abaixo).
  function perfilRecebeNotificacaoAceiteChamado(perfilId) {
    return _nivelDoItemDeNotificacao(perfilId, itensPermissao.ITEM_NOTIFICACAO_ACEITE_CHAMADO) === 'total';
  }

  // Mesma ideia, pro item GENÉRICO "Notificar Atualização de Etiqueta"
  // (ver catálogo, lib/itens-permissao.js, e
  // notificarAtualizacaoChamadoCorretivo/notificarAtualizacaoManutencaoProgramada,
  // abaixo).
  function perfilRecebeNotificacaoAtualizacaoEtiqueta(perfilId) {
    return _nivelDoItemDeNotificacao(perfilId, itensPermissao.ITEM_NOTIFICACAO_ATUALIZACAO_ETIQUETA) === 'total';
  }
  ...
  // Mesma ideia, pro item "Notificar Aceite de Chamado" — recalculado a
  // cada aceite, mesmo motivo do comentário acima.
  function usuariosParaNotificarAceiteChamado() {
    return _lerUsuarios()
      .filter(u => perfilRecebeNotificacaoAceiteChamado(u.perfil))
      .map(u => u.nomeUsuario);
  }

  // Mesma ideia, pro item GENÉRICO "Notificar Atualização de Etiqueta" —
  // recalculado a cada salvamento de etiqueta (corretiva ou programada),
  // mesmo motivo do comentário acima.
  function usuariosParaNotificarAtualizacaoEtiqueta() {
    return _lerUsuarios()
      .filter(u => perfilRecebeNotificacaoAtualizacaoEtiqueta(u.perfil))
      .map(u => u.nomeUsuario);
  }
  ...
  // Dispara a notificação de "chamado aceito" — pra todo mundo com a
  // permissão "Notificar Aceite de Chamado" marcada (padrão: Supervisão,
  // Encarregado ou Administrador — pedido do usuário: hoje só existia
  // aviso de ABERTURA de chamado, não de quando a Manutenção de fato
  // aceita e começa a trabalhar nele), EXCETO quem aceitou agora
  // (`nomeAutor`). Chamado SÓ na TRANSIÇÃO pra aceito (ver
  // /manutencao/aceitar-corretiva, lib/rotas/manutencao.js, que faz essa
  // checagem — aceito !== 'Sim' — antes de chamar esta função; aqui
  // dispara sem reavaliar a condição, mesma separação de
  // responsabilidades dos outros eventos: rota decide QUANDO, este
  // módulo decide PRA QUEM e O QUÊ). Fire-and-forget, mesmo motivo de
  // sempre — não pode atrasar nem falhar o aceite em si.
  function notificarAceiteChamado(chamado, nomeAutor) {
    const resumo = `${chamado.setor || 'Setor'} / ${chamado.maquina || 'Máquina'} — aceito por ${nomeAutor || 'Manutenção'}`;
    _dispararParaUsuarios(usuariosParaNotificarAceiteChamado(), nomeAutor, {
      titulo: 'Chamado de manutenção aceito',
      resumo,
      tag: `manutencao-aceite-${chamado.id}`,
      chamadoId: chamado.id,
    });
  }

  // Dispara a notificação GENÉRICA de "atualização" num CHAMADO
  // CORRETIVO — pra todo mundo com a permissão "Notificar Atualização
  // de Etiqueta" marcada, EXCETO quem salvou agora (`nomeAutor`). Só
  // chamada pela rota quando o salvamento é um UPDATE (chamado já
  // existia) e NENHUMA das notificações específicas de cima (pedido de
  // peça / peça recebida) disparou pra este mesmo salvamento — ver
  // lib/rotas/manutencao.js, POST /manutencao/corretiva, que faz essa
  // checagem antes de chamar esta função; aqui dispara sem reavaliar a
  // condição, mesma separação de responsabilidades das demais funções
  // deste arquivo: rota decide QUANDO, este módulo decide PRA QUEM e O
  // QUÊ. Fire-and-forget, mesmo motivo de sempre — não pode atrasar nem
  // falhar o salvamento em si. O resumo reflete se a etiqueta foi
  // FECHADA nesta atualização (mais relevante que só "atualizado").
  function notificarAtualizacaoChamadoCorretivo(chamado, nomeAutor) {
    const status = chamado.etiquetaFechada ? 'chamado fechado' : 'chamado atualizado';
    const resumo = `${chamado.setor || 'Setor'} / ${chamado.maquina || 'Máquina'} — ${status}`;
    _dispararParaUsuarios(usuariosParaNotificarAtualizacaoEtiqueta(), nomeAutor, {
      titulo: 'Atualização em chamado de manutenção',
      resumo,
      tag: `manutencao-atualizacao-${chamado.id}`,
      chamadoId: chamado.id,
    });
  }

  // Mesma ideia de notificarAtualizacaoChamadoCorretivo, acima, só que
  // pra um AGENDAMENTO PROGRAMADO — pedido do usuário, mesmo raciocínio:
  // cobre updates (aprovar/reprovar/executar/editar) que hoje não
  // disparavam nenhum aviso (só a CRIAÇÃO disparava, ver
  // notificarManutencaoProgramada, acima). Só chamada pela rota quando o
  // salvamento é um UPDATE (agendamento já existia) — ver
  // lib/rotas/manutencao.js, POST /manutencao/programada. `paramUrl:
  // 'programada'` de propósito, mesmo motivo de notificarManutencaoProgramada:
  // o front abre o AGENDAMENTO específico, não um chamado.
  function notificarAtualizacaoManutencaoProgramada(agendamento, nomeAutor) {
    ...
    _dispararParaUsuarios(usuariosParaNotificarAtualizacaoEtiqueta(), nomeAutor, {
      titulo: 'Atualização em manutenção programada',
      resumo,
      tag: `manutencao-programada-atualizacao-${agendamento.id}`,
  ...
  // "Notificação quando o PDF fica pronto" — DIFERENTE de todo o resto
  // deste arquivo: as demais funções `notificar*` avisam um GRUPO de
  // pessoas (perfis com a permissão marcada) e sempre EXCLUEM quem causou
  // o evento (`nomeAutor`). Aqui o destinatário é sempre UM usuário só —
  // exatamente quem pediu a exportação (`job.usuarioNome`, ver
  // lib/rotas/exportar-pdf.js) — e é justamente ELE que precisa ser
  // avisado (é o próprio "autor" do evento). Por isso não reaproveita
  // `_dispararParaUsuarios` (que sempre filtra o autor da lista): busca
  // direto as inscrições deste único usuário.
  //
  // Sem permissão nenhuma envolvida (diferente de "Notificar Abertura de
  // Chamado" etc., que dependem de lib/itens-permissao.js) — qualquer
  // usuário cadastrado que tenha ativado notificações push no próprio
  // dispositivo (ver public/js/notificacoes-push.js) recebe o aviso do
  // PDF que ELE MESMO pediu, não é uma notificação de "equipe".
  //
  // Fire-and-forget, mesmo motivo de sempre (ver notificarAberturaChamado,
  // acima) — chamado de dentro de `_concluirJob` (lib/rotas/
  ...
  function notificarPdfPronto(usuarioNome, nomeArquivo, jobId) {
    ...
    const payload = JSON.stringify({
      ...
      // Mesmo mecanismo de deep-link das demais notificações (ver
      // _dispararParaUsuarios, acima) — o front (app-core.js) lê o
      // parâmetro 'pdfPronto' no boot/no listener de 'message' do
      // service worker e abre o popover de Baixar/Descartar direto,
      // sem precisar a pessoa procurar o badge na topbar sozinha.
  ...
  return {
    ...
    perfilRecebeNotificacaoAceiteChamado,
    usuariosParaNotificarAceiteChamado,
    notificarAceiteChamado,
    perfilRecebeNotificacaoAtualizacaoEtiqueta,
    usuariosParaNotificarAtualizacaoEtiqueta,
    notificarAtualizacaoChamadoCorretivo,
    notificarAtualizacaoManutencaoProgramada,
