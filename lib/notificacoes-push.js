...
module.exports = function criarNotificacoesPush({
  ...
  function notificarAberturaChamado(chamado, nomeAutor) {
    ...
    const usuarios = usuariosParaNotificarAberturaChamado()
      .filter(nome => (nome || '').trim().toLowerCase() !== autorNormalizado);
    ...
    const payload = JSON.stringify({
      ...
      // direto na tela de Aceitar/Recusar deste chamado específico, em
      // vez de só cair no Menu (ver MAN.abrirChamado, manutencao.js).
      url: `/index.html?chamado=${encodeURIComponent(chamado.id)}`,
