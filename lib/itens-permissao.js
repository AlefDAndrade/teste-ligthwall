// ─── lib/itens-permissao.js — Catálogo de itens permissionáveis ────────────
// Lista TODO item que um "Novo Tipo de Perfil" customizado (ver
// Configurações → Usuários → "+ Criar novo tipo de perfil") pode marcar
// como Acesso Total / Apenas Visualizar / Ocultar. Cobre páginas,
// dashboards, abas de Configurações, sub-itens (Setor de Qualidade,
// Manutenção) e ações transversais ("Outros").
//
// Cada item tem:
//   id ........ mesmo id usado em data-page/showPage quando aplicável
//               (pra reaproveitar a infraestrutura já existente de
//               mostrar/esconder telas); id novo e só nosso quando não
//               existe página própria (ex: ações de "Outros").
//   rotulo .... texto exibido no modal de criação de perfil e em badges.
//   tipo ...... 'pagina' | 'sub' | 'dashboard' | 'config' | 'acao' — só
//               organiza visualmente o catálogo em grupos, não muda a
//               lógica de permissão em si.
//   pai ....... id do item pai, se for um sub-item (Setor de Qualidade e
//               Manutenção têm sub-itens; Manutenção Corretiva por sua vez
//               tem 4 sub-seções do próprio formulário de chamado).
//   area ...... quando presente, é a área de edição (ver lib/perfis.js,
//               AREAS_DE_EDICAO) que este item CONCEDE quando marcado como
//               "Acesso Total" — é a ponte entre o nível granular
//               (total/visualizar/ocultar) escolhido no catálogo e a
//               validação de verdade já existente no servidor
//               (podeEditarArea, server.js). Itens sem "area" (a maioria
//               dos dashboards, "Outros", e páginas puramente informativas)
//               não têm um conceito de edição no backend — a distinção
//               "Total vs Visualizar" pra eles é só visual (mostra/some ou
//               habilita/desabilita o controle na tela), conforme decidido:
//               enforcement de verdade no servidor fica só nas 5 áreas já
//               existentes (injetora, paradas, qualidade, manutencao,
//               manutencao-chamado); o resto é validado apenas no front por
//               enquanto.
const CATALOGO = [
  // ── Páginas ──────────────────────────────────────────────────────────
  { id: 'operacao', rotulo: 'Registrar Operação', tipo: 'pagina', area: 'injetora' },
  { id: 'turnos', rotulo: 'Desempenho por Turno', tipo: 'pagina' },
  { id: 'registro', rotulo: 'Relatório de Bateria', tipo: 'pagina', area: 'injetora' },
  { id: 'relatorio', rotulo: 'Relatório de Injeção', tipo: 'pagina', area: 'injetora' },
  { id: 'relatorio-bercos', rotulo: 'Relatório de Berços', tipo: 'pagina' },
  { id: 'metas', rotulo: 'Metas', tipo: 'pagina' },
  { id: 'paradas', rotulo: 'Registro de Paradas', tipo: 'pagina', area: 'paradas' },
  // ─── Páginas que EXISTIAM no app e nos perfis FIXOS (ver
  // PAGINAS_DE_TRABALHO, lib/perfis.js) mas nunca tinham entrado neste
  // catálogo — achado numa auditoria ("existem novas pages e
  // funcionalidades que ainda não estão lá"). Consequência do bug: como
  // paginasPermitidas() (lib/perfis-customizados.js) deriva a lista de
  // páginas SÓ do catálogo, um perfil CUSTOMIZADO nunca conseguia ver
  // estas telas — não apareciam no formulário de criação de perfil, logo
  // não havia como marcá-las, logo ficavam invisíveis pra sempre. Perfis
  // FIXOS nunca foram afetados (a lista deles é hardcoded à parte).
  //
  // Traços Descartados: 'injetora' porque editar/excluir um descarte já
  // registrado exige essa área no servidor (ver
  // lib/rotas/tracos-descartados.js) — mesma área do registro original.
  { id: 'tracos-descartados', rotulo: 'Traços Descartados (Perda)', tipo: 'pagina', area: 'injetora' },
  // One Page Report: sem "area" de propósito — a escrita ali
  // (comentários/Assuntos Gerais) exige sessaoOuAdmin, não uma das 5
  // áreas de edição (ver lib/rotas/one-page-report.js), então a
  // distinção Total/Visualizar aqui é só visual, igual aos dashboards.
  { id: 'one-page-report', rotulo: 'One Page Report', tipo: 'pagina' },

  { id: 'setor-qualidade', rotulo: 'Setor de Qualidade', tipo: 'pagina' },
  { id: 'qualidade-avaliacao', rotulo: 'Avaliação', tipo: 'sub', pai: 'setor-qualidade', area: 'qualidade' },
  { id: 'qualidade-dashboard', rotulo: 'Dashboard', tipo: 'sub', pai: 'setor-qualidade' },
  { id: 'qualidade-registro', rotulo: 'Registros', tipo: 'sub', pai: 'setor-qualidade' },

  { id: 'manutencao', rotulo: 'Manutenção', tipo: 'pagina' },
  { id: 'manutencao-corretiva', rotulo: 'Corretiva', tipo: 'sub', pai: 'manutencao' },
  { id: 'manutencao-abertura', rotulo: 'Abertura de Chamado', tipo: 'sub', pai: 'manutencao-corretiva', area: 'manutencao-chamado' },
  { id: 'manutencao-detalhes-problema', rotulo: 'Detalhes do Problema', tipo: 'sub', pai: 'manutencao-corretiva', area: 'manutencao-chamado' },
  { id: 'manutencao-execucao', rotulo: 'Execução da Manutenção', tipo: 'sub', pai: 'manutencao-corretiva', area: 'manutencao' },
  { id: 'manutencao-acompanhamento-supervisao', rotulo: 'Acompanhamento da Supervisão', tipo: 'sub', pai: 'manutencao-corretiva', area: 'manutencao' },
  { id: 'manutencao-programada', rotulo: 'Programada', tipo: 'sub', pai: 'manutencao', area: 'manutencao' },
  { id: 'manutencao-visao-executiva', rotulo: 'Visão Executiva', tipo: 'sub', pai: 'manutencao' },
  // Permissão de NOTIFICAÇÃO (não de edição) — ver lib/notificacoes-push.js
  // e conversa que motivou isso: "toda vez que um chamado for aberto, quem
  // tem perfil de manutenção vai ser notificado". Reaproveita o mesmo
  // catálogo/tela de "Acesso Total / Apenas Visualizar / Ocultar" que já
  // existe pra páginas — aqui os níveis significam outra coisa:
  // 'total' = ESTE PERFIL RECEBE a notificação push de novo chamado
  // aberto; 'visualizar'/'ocultar' = não recebe (tratados igual, ver
  // notificacoes-push.js). Sem "area" de propósito — não concede
  // nenhuma permissão de EDIÇÃO, só controla quem é avisado (ver
  // permissoesPadraoDoPerfilFixo, lib/perfis.js, que trata este id à
  // parte pra decidir o padrão de cada perfil fixo).
  { id: 'manutencao-notificacao-abertura', rotulo: 'Notificar Abertura de Chamado', tipo: 'sub', pai: 'manutencao-corretiva' },
  // Mesmo espírito do item acima, só que pro momento "chamado JÁ em
  // execução (situacao='Em Manutencao') teve um PEDIDO DE PEÇA aberto
  // (aguardandoPecas passou a 'Sim')" — pedido do usuário. 'total' =
  // ESTE PERFIL RECEBE a notificação; 'visualizar'/'ocultar' = não
  // recebe (ver notificacoes-push.js). Sem "area" pelo mesmo motivo do
  // item de abertura: só controla quem é avisado, nenhuma permissão de
  // EDIÇÃO nova.
  { id: 'manutencao-notificacao-pedido-peca', rotulo: 'Notificar Pedido de Peça', tipo: 'sub', pai: 'manutencao-corretiva' },
  // Mesmo espírito dos dois itens acima, pro momento "chamado salvo com
  // Status da Compra = 'Peça recebida'" (ver campo statusCompra, Seção 4
  // do chamado) — pedido do usuário: Manutenção, Administrador,
  // Supervisão e Encarregado devem ser avisados assim que a peça chegar,
  // pra dar seguimento ao conserto. 'total' = ESTE PERFIL RECEBE a
  // notificação; 'visualizar'/'ocultar' = não recebe (ver
  // notificacoes-push.js). Sem "area" pelo mesmo motivo dos itens acima:
  // só controla quem é avisado, nenhuma permissão de EDIÇÃO nova.
  { id: 'manutencao-notificacao-peca-recebida', rotulo: 'Notificar Peça Recebida', tipo: 'sub', pai: 'manutencao-corretiva' },
  // Mesmo espírito dos três itens acima, pro momento "um agendamento de
  // MANUTENÇÃO PROGRAMADA é criado" (ver POST /manutencao/programada,
  // lib/rotas/manutencao.js) — pedido do usuário: DIFERENTE dos itens
  // acima (que restringem por perfil), aqui o padrão de TODO perfil
  // fixo é 'total' (ver permissoesPadraoDoPerfilFixo, lib/perfis.js) —
  // todo mundo é avisado quando uma manutenção é agendada, continua
  // ajustável depois pela mesma tela de Configurações → Notificações,
  // caso alguém queira restringir pra um perfil específico. 'total' =
  // ESTE PERFIL RECEBE a notificação; 'visualizar'/'ocultar' = não
  // recebe (ver notificacoes-push.js). Sem "area" pelo mesmo motivo dos
  // itens acima: só controla quem é avisado, nenhuma permissão de
  // EDIÇÃO nova.
  { id: 'manutencao-notificacao-programada', rotulo: 'Notificar Manutenção Programada Agendada', tipo: 'sub', pai: 'manutencao-corretiva' },
  // Mesmo espírito dos itens acima, pro momento "chegou o DIA do
  // agendamento (status='Aprovado') e já passou das 09h da manhã" (ver
  // executarLembreteManutencaoProgramadaSeNecessario,
  // lib/notificacoes-push.js) — pedido do usuário: um lembrete no dia,
  // separado do aviso de "agendamento criado" (item acima, disparado no
  // momento em que alguém cria o agendamento, dias antes). Mesmo padrão
  // do item de cima: por padrão TODO perfil fixo é 'total' (ver
  // permissoesPadraoDoPerfilFixo, lib/perfis.js), ajustável depois pela
  // mesma tela de Configurações → Notificações. 'total' = ESTE PERFIL
  // RECEBE o lembrete; 'visualizar'/'ocultar' = não recebe (ver
  // notificacoes-push.js). Sem "area" pelo mesmo motivo dos itens acima:
  // só controla quem é avisado, nenhuma permissão de EDIÇÃO nova.
  { id: 'manutencao-notificacao-programada-lembrete', rotulo: 'Notificar Lembrete de Manutenção Programada (no dia)', tipo: 'sub', pai: 'manutencao-corretiva' },
  // Mesmo espírito dos itens acima, pro momento "chamado ACEITO" (ver
  // POST /manutencao/aceitar-corretiva, lib/rotas/manutencao.js) —
  // pedido do usuário: hoje só existe aviso de ABERTURA de chamado, não
  // de quando a Manutenção de fato aceita e começa a trabalhar nele.
  // Quem recebe por padrão é Supervisão, Encarregado ou Administrador —
  // mesmo grupo de "Notificar Pedido de Peça" acima: são os perfis que
  // acompanham o andamento do chamado, não quem executa (Manutenção já
  // sabe que aceitou, é quem disparou a ação). 'total' = ESTE PERFIL
  // RECEBE a notificação; 'visualizar'/'ocultar' = não recebe (ver
  // notificacoes-push.js). Sem "area" pelo mesmo motivo dos itens
  // acima: só controla quem é avisado, nenhuma permissão de EDIÇÃO
  // nova.
  { id: 'manutencao-notificacao-aceite', rotulo: 'Notificar Aceite de Chamado', tipo: 'sub', pai: 'manutencao-corretiva' },
  // Mesmo espírito dos itens acima, mas GENÉRICO: qualquer ATUALIZAÇÃO
  // salva numa etiqueta (chamado corretivo OU agendamento programado)
  // que não seja coberta por um dos eventos específicos de cima
  // (abertura/pedido de peça/peça recebida/aceite/agendamento criado) —
  // pedido do usuário: "notificar todas as atualizações que forem
  // feitas em uma etiqueta, tanto corretiva como programada". Cobre,
  // por exemplo: editar Abertura/Detalhes/Execução/Acompanhamento de um
  // chamado já existente, fechar a etiqueta (Concluído), e
  // aprovar/reprovar/executar/editar um agendamento já existente (ver
  // lib/rotas/manutencao.js, POST /manutencao/corretiva e POST
  // /manutencao/programada — cada rota decide, na hora de salvar, se
  // este evento já disparou uma notificação específica; só cai aqui se
  // não disparou nenhuma). NÃO cobre exclusão (excluir-corretiva/
  // excluir-programada) — pedido do usuário, de propósito, só updates.
  // 'total' = ESTE PERFIL RECEBE a notificação; 'visualizar'/'ocultar' =
  // não recebe (ver notificacoes-push.js). Sem "area" pelo mesmo motivo
  // dos itens acima: só controla quem é avisado, nenhuma permissão de
  // EDIÇÃO nova.
  { id: 'manutencao-notificacao-atualizacao', rotulo: 'Notificar Atualização de Etiqueta', tipo: 'sub', pai: 'manutencao-corretiva' },


  { id: 'tv', rotulo: 'Modo TV', tipo: 'pagina' },

  // ── Dashboards ───────────────────────────────────────────────────────
  { id: 'analise-operacional', rotulo: 'Análise Operacional', tipo: 'dashboard' },
  { id: 'qualidade-tracos', rotulo: 'CEP', tipo: 'dashboard' },
  // Tela auxiliar do CEP (README, pedido — "consulta detalhada dos
  // traços produzidos no período"): lista de traços + detalhe de
  // insumos por traço + exportação Excel. Item PRÓPRIO (não amarrado a
  // 'qualidade-tracos') — um Administrador pode querer liberar só o CEP
  // agregado sem abrir o detalhe de insumo por traço, ou vice-versa.
  { id: 'consulta-tracos', rotulo: 'Consulta de Traços (Insumos)', tipo: 'dashboard' },
  { id: 'analise-focada', rotulo: 'Análise Focada / Rastreabilidade', tipo: 'dashboard' },
  { id: 'analise-bercos', rotulo: 'Análise de Berços', tipo: 'dashboard' },
  { id: 'oee', rotulo: 'OEE', tipo: 'dashboard' },

  // ── Outros ───────────────────────────────────────────────────────────
  { id: 'importar-documentos', rotulo: 'Importar Documentos', tipo: 'acao' },
  { id: 'export-interativo', rotulo: 'Exportações Interativas', tipo: 'acao' },
  { id: 'export-excel', rotulo: 'Exportações de Excel', tipo: 'acao' },
  { id: 'edicao-dados', rotulo: 'Edição dos Dados', tipo: 'acao' },
  { id: 'backup-restauracao', rotulo: 'Backup e Restauração', tipo: 'acao' },

  // ── Configurações ────────────────────────────────────────────────────
  { id: 'config-dados', rotulo: 'Bateria e Montagem', tipo: 'config' },
  // Paletes (Configurações → Paletes) — mesma omissão de
  // 'tracos-descartados'/'one-page-report' acima: a aba existe no modal
  // (cfg-nav-paletes, modal-config.html), os perfis FIXOS já a conhecem
  // (ABAS_CONFIG_ADMIN, lib/perfis.js) e o front já pergunta por ela
  // (_cfgAplicarVisibilidadeDeAbas checa 'config-paletes',
  // public/js/app-core.js) — só o catálogo não tinha, então a aba ficava
  // permanentemente escondida de qualquer perfil customizado, sem forma
  // nenhuma de liberar.
  { id: 'config-paletes', rotulo: 'Paletes', tipo: 'config' },
  { id: 'config-atalhos', rotulo: 'Atalho de Teclados', tipo: 'config' },
  { id: 'config-usuarios', rotulo: 'Usuários', tipo: 'config' },
  { id: 'config-autorizados', rotulo: 'Operação em Andamento', tipo: 'config' },
  // Motivos de Parada (Configurações → Motivos de Parada) — mesmo
  // raciocínio de config-dados, acima: lista de motivos configurável em
  // vez de fixa no código (ver public/js/paradas.js, MOTIVO_PARADA_OPTS/
  // LW.MOTIVO_PARADA_OPTS, data.js). 'tipo: config' de propósito (não
  // 'pagina'): mesma convenção do resto deste bloco — controla o acesso
  // À ABA de configuração, não à página de Registro de Paradas em si
  // (essa é 'paradas', em PAGINAS_DE_TRABALHO, lib/perfis.js — sempre
  // visível a todo mundo, edição sempre liberada, ver AREAS_DE_EDICAO).
  { id: 'config-paradas', rotulo: 'Motivos de Parada', tipo: 'config' },
  // Tipos de Manutenção (Configurações → Tipos de Manutenção) — mesmo
  // raciocínio de config-paradas, acima: lista configurável em vez de
  // fixa no código (ver public/js/manutencao.js,
  // carregarOpcoesTipoManutencao/LW.TIPO_MANUTENCAO_OPTS, data.js).
  { id: 'config-tipos-manutencao', rotulo: 'Tipos de Manutenção', tipo: 'config' },
  // Prioridade de Chamado (Configurações → Prioridades) — mesmo
  // raciocínio de config-tipos-manutencao, acima: lista configurável em
  // vez de fixa no código, só que cada item tem uma COR própria também
  // (ver public/js/manutencao.js, carregarOpcoesPrioridade/
  // LW.PRIORIDADE_OPTS, data.js).
  { id: 'config-prioridades', rotulo: 'Prioridades', tipo: 'config' },
  // Insumos de Receitas (Configurações → Insumos de Receitas) — mesmo
  // raciocínio de config-prioridades, acima: catálogo configurável em
  // vez de fixo no código (ver LW.INSUMO_RECEITA_OPTS, data.js).
  { id: 'config-insumos', rotulo: 'Insumos de Receitas', tipo: 'config' },
  { id: 'config-automacao', rotulo: 'Automação', tipo: 'config' },
  // Notificações (Configurações → Notificações) — mesma omissão de
  // 'config-paletes' acima: aba existente no modal, já conhecida pelos
  // perfis FIXOS (ABAS_CONFIG_ADMIN) e já consultada pelo front
  // (_cfgAplicarVisibilidadeDeAbas), mas ausente daqui — invisível pra
  // sempre a qualquer perfil customizado. Atenção: esta aba é o
  // CONTROLE de quem recebe push; os itens 'manutencao-notificacao-*'
  // (acima) são quem de fato RECEBE cada evento — coisas diferentes.
  { id: 'config-notificacoes', rotulo: 'Notificações', tipo: 'config' },
  { id: 'config-sql', rotulo: 'Dados SQL', tipo: 'config' },
  // Registro de Operação Offline (PWA) — itens 6/7 do plano (ver README):
  // aprovar/corrigir/recusar o que foi enviado por
  // POST /operacao-offline/enviar. Sempre visível ao Administrador
  // (ABAS_CONFIG_ADMIN, lib/perfis.js); um perfil CUSTOMIZADO só ganha
  // acesso se o Administrador marcar "Acesso Total" pra este item aqui —
  // útil pra delegar a revisão sem dar Administrador completo (ex.: um
  // Supervisor de confiança que só cuida disso).
  { id: 'config-operacoes-offline', rotulo: 'Operações a Validar (Offline)', tipo: 'config' },
];

const NIVEIS = ['total', 'visualizar', 'ocultar'];

const IDS_VALIDOS = new Set(CATALOGO.map(i => i.id));

// Itens que têm "area" — usados pra fazer a ponte entre o nível granular
// escolhido no catálogo e as 5 áreas de edição já validadas de verdade no
// servidor (ver lib/perfis.js, AREAS_DE_EDICAO).
const ITENS_POR_AREA = {};
for (const item of CATALOGO) {
  if (!item.area) continue;
  (ITENS_POR_AREA[item.area] = ITENS_POR_AREA[item.area] || []).push(item.id);
}

function itemValido(id) {
  return IDS_VALIDOS.has(id);
}

// Valida um mapa {itemId: nivel} contra o catálogo — usado tanto por
// perfis CUSTOMIZADOS (lib/perfis-customizados.js) quanto por
// OVERRIDES de perfis FIXOS (lib/perfis-fixos-overrides.js, ver
// conversa que motivou a mudança — engrenagem ao lado do campo "Perfil"
// em Configurações → Usuários). Itens não mencionados no payload viram
// 'ocultar' por padrão — quem salva decide explicitamente o que
// liberar, em vez de herdar acesso a algo que esqueceu de marcar.
function validarMapaDePermissoes(permissoes) {
  if (!permissoes || typeof permissoes !== 'object' || Array.isArray(permissoes)) {
    throw new Error('Campo "permissoes" precisa ser um objeto {itemId: nivel}.');
  }
  const limpo = {};
  for (const [itemId, nivel] of Object.entries(permissoes)) {
    if (!itemValido(itemId)) {
      throw new Error(`Item de permissão "${itemId}" não existe no catálogo.`);
    }
    if (!NIVEIS.includes(nivel)) {
      throw new Error(`Item "${itemId}": nível "${nivel}" inválido. Precisa ser um de: ${NIVEIS.join(', ')}.`);
    }
    limpo[itemId] = nivel;
  }
  for (const item of CATALOGO) {
    if (!(item.id in limpo)) limpo[item.id] = 'ocultar';
  }
  return limpo;
}

// Id do item de notificação de abertura de chamado (ver CATALOGO, acima)
// exportado como constante — evita espalhar a string mágica
// 'manutencao-notificacao-abertura' por lib/perfis.js e
// lib/notificacoes-push.js.
const ITEM_NOTIFICACAO_ABERTURA_CHAMADO = 'manutencao-notificacao-abertura';

// Id do item de notificação de PEDIDO DE PEÇA (ver CATALOGO, acima) —
// mesmo raciocínio do de cima, string mágica isolada aqui em vez de
// espalhada por lib/perfis.js e lib/notificacoes-push.js.
const ITEM_NOTIFICACAO_PEDIDO_PECA = 'manutencao-notificacao-pedido-peca';

// Id do item de notificação de PEÇA RECEBIDA (ver CATALOGO, acima) — mesmo
// raciocínio dos dois de cima, string mágica isolada aqui em vez de
// espalhada por lib/perfis.js e lib/notificacoes-push.js.
const ITEM_NOTIFICACAO_PECA_RECEBIDA = 'manutencao-notificacao-peca-recebida';

// Id do item de notificação de MANUTENÇÃO PROGRAMADA AGENDADA (ver
// CATALOGO, acima) — mesmo raciocínio dos três de cima, string mágica
// isolada aqui em vez de espalhada por lib/perfis.js e
// lib/notificacoes-push.js.
const ITEM_NOTIFICACAO_MANUTENCAO_PROGRAMADA = 'manutencao-notificacao-programada';

// Id do item de notificação de LEMBRETE DE MANUTENÇÃO PROGRAMADA NO DIA
// (ver CATALOGO, acima) — mesmo raciocínio dos itens de cima, string
// mágica isolada aqui em vez de espalhada por lib/perfis.js e
// lib/notificacoes-push.js.
const ITEM_NOTIFICACAO_MANUTENCAO_PROGRAMADA_LEMBRETE = 'manutencao-notificacao-programada-lembrete';

// Id do item de notificação de ACEITE DE CHAMADO (ver CATALOGO, acima) —
// mesmo raciocínio dos itens de cima, string mágica isolada aqui em vez
// de espalhada por lib/perfis.js e lib/notificacoes-push.js.
const ITEM_NOTIFICACAO_ACEITE_CHAMADO = 'manutencao-notificacao-aceite';

// Id do item de notificação GENÉRICA de ATUALIZAÇÃO DE ETIQUETA (ver
// CATALOGO, acima) — mesmo raciocínio dos itens de cima, string mágica
// isolada aqui em vez de espalhada por lib/perfis.js e
// lib/notificacoes-push.js.
const ITEM_NOTIFICACAO_ATUALIZACAO_ETIQUETA = 'manutencao-notificacao-atualizacao';

module.exports = {
  CATALOGO, NIVEIS, ITENS_POR_AREA, itemValido, validarMapaDePermissoes,
  ITEM_NOTIFICACAO_ABERTURA_CHAMADO, ITEM_NOTIFICACAO_PEDIDO_PECA, ITEM_NOTIFICACAO_PECA_RECEBIDA,
  ITEM_NOTIFICACAO_MANUTENCAO_PROGRAMADA, ITEM_NOTIFICACAO_MANUTENCAO_PROGRAMADA_LEMBRETE,
  ITEM_NOTIFICACAO_ACEITE_CHAMADO, ITEM_NOTIFICACAO_ATUALIZACAO_ETIQUETA,
};
