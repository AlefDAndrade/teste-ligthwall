...
module.exports = function criarPermissoesArea({ sessao, sessaoUsuario, perfis, perfisFixosOverrides, perfisCustomizados }) {
  ...
  // no front; nome de cadastro pra qualquer outro usuário logado; null se
  // não há sessão nenhuma. Usada pela inscrição de notificações push
  // (lib/rotas/notificacoes.js) pra saber de quem é cada inscrição — só
  // isso, não decide permissão nenhuma (essa função só identifica QUEM
  // está pedindo, não SE pode).
  // Histórico: já se chamou `nomeDeQuemAceita`, de quando também
  // identificava quem podia aceitar/recusar/excluir um chamado do Setor
  // de Manutenção (descontinuado, ver lib/perfis.js) — renomeada porque
  // não tem mais nada de "aceitar" nela, é só "quem está logado".
  function nomeDeQuemEstaLogado(req) {
    const dados = sessaoUsuario.dadosDaSessao(req);
