...
//     fase, feito, total, segundosRestantes, nomeArquivo, pdfBuffer, erro,
//     page, criadoEm, concluidoEm, clientesSse: Set<ServerResponse> }
const _jobs = new Map();
  ...
  async function _processarJob(job, html, puppeteer, executavel) {
    ...
    try {
      ...
      await page.exposeFunction('__afReportarProgresso', (feito, total) => {
        console.log(`[DEBUG exportar-pdf] __afReportarProgresso chamado: feito=${feito} total=${total} (job ${job.id})`);
        _atualizarProgresso(job, 'ajustando', feito, total);
      });
      // DEBUG temporário — captura erros/console DENTRO da página headless
      // (setContent não propaga exceptions do JS injetado pro servidor por
      // padrão; sem isto, um erro no script de ajuste de escala é
      // invisível tanto no browser do usuário quanto no log do servidor).
      page.on('pageerror', (err) => {
        console.error(`[DEBUG exportar-pdf] erro DENTRO da página headless (job ${job.id}):`, err.message);
      });
      page.on('console', (msg) => {
        if (msg.type() === 'error') {
          console.error(`[DEBUG exportar-pdf] console.error DENTRO da página headless (job ${job.id}):`, msg.text());
        }
      });
      ...
      const totalPaginasConhecido = job.total > 0 ? job.total : null;
      console.log(`[DEBUG exportar-pdf] job.total capturado ANTES de imprimir: ${job.total} → totalPaginasConhecido=${totalPaginasConhecido} (job ${job.id})`);
