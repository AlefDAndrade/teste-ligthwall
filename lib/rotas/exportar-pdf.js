...
function _atualizarProgresso(job, fase, feito, total, segundosRestantes) {
  ...
  _emitirEvento(job, 'progresso', { fase, feito, total, segundosRestantes: job.segundosRestantes });
}
...
module.exports = function criarRotasExportarPdf() {
  ...
  async function _processarJob(job, html, puppeteer, executavel) {
    ...
    try {
      ...
      // `job.total` — é o total de operações que o ajuste de escala reportou
      // (ou 1, pra Simples/qualquer HTML sem esse mecanismo), usado como
      // base da estimativa de tempo do ticker abaixo.
      const totalOperacoesParaEstimativa = job.total || 1;
