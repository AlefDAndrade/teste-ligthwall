...
module.exports = function criarRotasExportarPdf({ db, PRIVATE_DIR, sessaoUsuario } = {}) {
  ...
  return function tentar(req, res, urlPath) {
    ...
    //
    // Etapa 2 (ver comentário no topo do arquivo): esta rota deixou de
    // apagar o job/arquivo automaticamente depois de servir o download —
    // baixar não é "decidir" (a pessoa pode querer baixar de novo). Só
    // `POST /descartar/:jobId`, logo abaixo, libera o usuário pra gerar
    // outro PDF (o TTL de 7 dias em disco continua como rede de
    // segurança pra quem nunca decide nada).
