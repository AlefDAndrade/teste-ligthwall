...
// Rotas:
//   POST /exportar-pdf/iniciar              { html: string, filename?: string, landscape?: boolean } → { ok, jobId }
...
function _criarJob(usuarioId, usuarioNome) {
  ...
  const job = {
    ...
    nomeArquivo: null,
    // Orientação da página impressa — `false` (padrão, retrato) preserva o
    // comportamento de sempre; setado a partir de `payload.landscape` em
    // POST /iniciar (ver mais abaixo). Hoje só o One Page Report pede
    // paisagem (public/js/one-page-report.js, _gerarHtmlOprStandalone),
    // mas o campo é genérico — qualquer export futuro pode usar.
    landscape: false,
    pdfBuffer: null,
    erro: null,
    page: null,
...
module.exports = function criarRotasExportarPdf({ db, PRIVATE_DIR, sessaoUsuario, sessao, notificarPdfPronto, urlBaseLocal } = {}) {
  ...
  async function _processarJob(job, html, puppeteer, executavel, notificarPdfPronto) {
    ...
    try {
      ...
      // Fontes locais: o HTML do PDF embute css/styles.css (ex.: One Page
      // Report), que faz @import de '/css/fontes.css' — endereço relativo,
      // que não resolve numa página montada com setContent (about:blank).
      // Aponta pro próprio servidor pela rede local da máquina. Antes as
      // fontes vinham do Google Fonts, o que exigia internet no servidor.
      const htmlComFontes = urlBaseLocal
        ? html.split("url('/css/fontes.css')").join(`url('${urlBaseLocal}/css/fontes.css')`)
        : html;
      await page.setContent(htmlComFontes, { waitUntil: 'networkidle0', timeout: 0 });
      if (job.status !== 'processando') return;
  ...
  return function tentar(req, res, urlPath) {
    ...
    if (req.method === 'POST' && urlPath === '/exportar-pdf/iniciar') {
      ...
      req.on('end', () => {
        ...
        // Opcional — ver comentário em _criarJob, acima. `=== true`
        // proposital: qualquer coisa que não seja o booleano `true`
        // (undefined, string, número solto de um payload malformado)
        // cai no padrão retrato, nunca "quase verdadeiro" por acidente.
        job.landscape = payload.landscape === true;
        if (db) {
