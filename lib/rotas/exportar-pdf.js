...
// precisa ter um DONO (`job.usuarioId`): é o que a Etapa 2 (bloqueio de
// "só um PDF pendente por usuário") e o aviso "PDF pronto" ao voltar no
// site vão usar pra saber de quem é cada job.
//
...
function _concluirJob(job, pdfBuffer, db, pdfsDir) {
...
module.exports = function criarRotasExportarPdf({ db, PRIVATE_DIR, sessaoUsuario } = {}) {
  ...
  async function _processarJob(job, html, puppeteer, executavel) {
    ...
    try {
      ...
      _concluirJob(job, pdfBuffer, db, PDFS_DIR);
    } catch (err) {
  ...
  return function tentar(req, res, urlPath) {
    ...
    if (req.method === 'POST' && urlPath === '/exportar-pdf/iniciar') {
      ...
      const dadosSessao = sessaoUsuario ? sessaoUsuario.dadosDaSessao(req) : null;
      if (!dadosSessao) {
      ...
      req.on('end', () => {
        ...
        const job = _criarJob(dadosSessao.usuarioId);
    ...
    if (req.method === 'GET' && urlPath.startsWith('/exportar-pdf/arquivo/')) {
      ...
      if (job) {
        ...
        _jobs.delete(jobId);
        if (db) { try { fs.unlinkSync(path.join(PDFS_DIR, jobId + '.pdf')); } catch (_) { /* pode nem ter chegado a escrever ainda */ } db.apagarExportacaoPdf(jobId); }
        return true;
      }
      ...
      try { fs.unlinkSync(registro.caminho_arquivo); } catch (_) { /* já entregue — segue mesmo se a limpeza falhar */ }
      db.apagarExportacaoPdf(jobId);
      return true;
    }
