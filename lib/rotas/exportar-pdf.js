  ...
  return function tentar(req, res, urlPath) {
    ...
    req.on('end', async () => {
      ...
      try {
        ...
        // Timeout de 15s (antes: 5s) — o script agora pode rodar até 6
        // passadas de correção de escala pra operações com MUITO conteúdo
        // na Avaliação de Qualidade (ver MAX_PASSADAS em
        // _afScriptAjustePaginaUnica, analise-focada.js); cada passada
        // força um reflow, então documentos grandes podem levar mais que
        // os 5s antigos pra convergir. 15s ainda é bem menor que o
        // timeout de 30s do próprio setContent acima, então não corre o
        // risco de mascarar um travamento real.
