...
// assim que alguém conecta); `dispositivoAutorizado`/
// `negarDispositivoNaoAutorizado`/`lerBercosAndamento`/
...
//
// `deviceId` é derivado aqui dentro a partir de queryParams (mesma
// expressão usada em server.js) — só estas rotas, neste módulo, precisam
// dele.
...
module.exports = function criarRotasOperacaoAndamento({
  ...
  return function tentar(req, res, urlPath, queryParams) {
    ...
    if (req.method === 'POST' && urlPath === '/admin/resetar-operacao') {
      ...
      //   autorizados (ver dispositivoAutorizado(), acima), então
