...
// server.js) — as duas juntas. `deviceId` (extraído aqui embaixo) tem
// portanto DOIS papéis agora: autorização (passado como 2º argumento pra
// podeControlarOperacao/negarControleDeOperacao) e identificar o "dono"
// da operação em andamento (evita dois computadores autorizados brigando
// pela mesma operação ao mesmo tempo, ver donoDeviceId abaixo).
    ...
    if (req.method === 'POST' && urlPath === '/salvar-operacao-andamento') {
      ...
      req.on('end', () => {
        try {
          ...
          if (!podeControlarOperacao(req, deviceId)) { negarControleDeOperacao(res, req, deviceId); return; }
