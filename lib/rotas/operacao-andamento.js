...
//
// `podeControlarOperacao`/`negarControleDeOperacao` (ver
// lib/permissoes-area.js) checam só a sessão de USUÁRIO logado (ver
// lib/sessao-usuario.js, lib/perfis.js) — a trava adicional por
// DISPOSITIVO que existia aqui foi removida. `deviceId` (extraído aqui
// embaixo) continua tendo um papel só agora: identificar o "dono" da
// operação em andamento (evita dois computadores brigando pela mesma
// operação ao mesmo tempo, ver donoDeviceId abaixo) — não afeta mais
// autorização nenhuma.
    ...
    if (req.method === 'POST' && urlPath === '/salvar-operacao-andamento') {
      ...
      req.on('end', () => {
        try {
          ...
          // idAndamento (opcional) — identifica QUAL operação o cliente está
          // tentando encerrar (gerado no início dela, ver iniciarInjecao(),
          // operacao.js, e carregado em todo persist() daí em diante). Só
          // é usado numa limpeza (ehLimpeza) — ver checagem logo abaixo.
          // Ausente = comportamento antigo, sem essa proteção extra (cobre
          // clientes com cache antigo de operacao.js/data.js, ou qualquer
          // outro chamador que nunca tenha mandado esse campo).
          const idAndamento = typeof payload.idAndamento === 'string' ? payload.idAndamento : null;
          ...
          // Limpeza CONDICIONAL — só existe pra fechar o buraco de quem caiu
          // de rede bem na hora de encerrar (ver README/conversa que motivou
          // esta mudança): o aviso "melhor esforço" de que a operação
          // terminou não chegou a sair do dispositivo, a operação já foi
          // registrada de verdade quando a conexão voltou (via fila
          // offline, ver LW.tentarSincronizarFilaPendentes, data.js), e só
          // NESSE reenvio tardio é que o idAndamento chega aqui. Se o que
          // está em andamento AGORA já não é mais essa mesma operação
          // (foi limpo por outro caminho nesse meio-tempo, ou — inclusive
          // no MESMO dispositivo — uma operação NOVA e legítima já começou
          // antes da fila antiga terminar de sincronizar), não faz NADA:
          // nem checa autorização, nem toca no disco, nem manda broadcast.
          // Sem isso, esse reenvio tardio poderia apagar por engano o
          // estado de uma operação nova que não tem nada a ver com a que
          // caiu de rede. Responde OK do mesmo jeito — do ponto de vista de
          // quem pediu, "essa operação não está mais pendente de aviso" já
          // está satisfeito, seja porque foi limpa agora, seja porque já
          // não era mais a atual.
          if (ehLimpeza && idAndamento && atual && atual.idAndamento !== idAndamento) {
            res.writeHead(200, { 'Content-Type': 'application/json' });
            res.end(JSON.stringify({ ok: true, ignorado: true }));
            return;
          }
          ...
          //
          // Administrador Master (sessão de senha mestra OU usuário logado
          // com perfil Administrativo — ambos cobertos por
          // sessao.requestTemSessaoValida aqui, que recebe sessaoOuAdmin, ver
          // server.js) NUNCA fica travado por essa disputa: pedido explícito
          // do usuário — o Master precisa poder pausar, trocar traço, marcar
          // berço etc. numa operação que outra pessoa iniciou, sem precisar
          // "forçar"/"Limpar Tudo" (que reseta tudo e tira o controle de
          // quem estava operando). Só ainda passa pela trava de DISPOSITIVO
          // autorizado (podeControlarOperacao, acima) — sem exceção pra
          // nenhum perfil, mesma regra de sempre.
          const donoAtual = (atual && typeof atual === 'object') ? (atual.donoDeviceId || null) : null;
          const souODono = !donoAtual || donoAtual === deviceId;
          const souMaster = sessao.requestTemSessaoValida(req);

          if (!souODono && !souMaster && !forcar) {
          ...
          // sempre recalculado aqui: mantém o dono atual (mesmo quando quem
          // gravou agora foi o Master driblando a trava acima — o Master
          // NÃO assume a posse, só tem permissão de mexer; a operação
          // continua do dono original assim que ele voltar a mandar dados),
          // ou assume este deviceId como novo dono se a operação estava vazia.
          let dadosFinal;
          if (ehLimpeza) {
    ...
    if (req.method === 'POST' && urlPath === '/marcar-berco-andamento') {
      ...
      req.on('end', () => {
        try {
          ...
          // Tipo de placa ('2p'/'sp'/...) que o CLIENTE já resolveu, na
          // montagem/grade de agora, pra este lado (ver LW.tipoDoLadoMontagem,
          // data.js) — opcional, string ou ausente/null. Só é usado (e
          // gravado) quando o lado está sendo MARCADO como 'nao_enchido';
          // fica FIXO dali em diante, mesmo que a montagem/grade mude depois
          // (ver aplicarNaoEnchidosNoCalc, data.js, e conversa que motivou
          // esta mudança — antes disso o desconto "seguia" a config atual e
          // podia trocar de tipo sozinho depois de uma reconfiguração).
          const tipoPedido = (payload && typeof payload.tipo === 'string' && payload.tipo) ? payload.tipo : null;
          ...
          // ver comentário lá (inclui o mesmo bypass do Master). Sem
          // operação vazia aqui pra "virar dono": a essa altura ela já
          // existe (checado acima), então só quem já é o dono (ou nenhum
          // dono foi definido ainda, caso raro), ou o Master, marca.
          const donoAtual = (typeof atual === 'object') ? (atual.donoDeviceId || null) : null;
          const souODono = !donoAtual || donoAtual === deviceId;
          const souMaster = sessao.requestTemSessaoValida(req);
          if (!souODono && !souMaster) {
            res.writeHead(409, { 'Content-Type': 'application/json' });
            res.end(JSON.stringify({
              ok: false,
          ...
          if (doBerco[lado] === 'baixou' || doBerco[lado] === 'nao_enchido') {
            ...
            // Desmarcar também limpa o tipo fixado daquele lado (ver
            // doBerco.tipos abaixo) — não faz sentido guardar um tipo
            // "fixado" pra um lado que voltou a 'okay'.
            if (doBerco.tipos) {
              delete doBerco.tipos[lado];
              if (!Object.keys(doBerco.tipos).length) delete doBerco.tipos;
            }
          } else {
            doBerco[lado] = estadoPedido;
            // Sidecar `tipos` — só grava quando o cliente mandou um tipo
            // resolvido (normalmente só ao marcar 'nao_enchido', ver
            // bateria-atual.js). Formato: { esquerda: 'sp', direita: '2p' }.
            // Ignorado por todo o resto do código que só lê doBerco.esquerda/
            // doBerco.direita (ver criarBercosVisuaisIniciais, db) — existe
            // só pra aplicarNaoEnchidosNoCalc (data.js) preferir sobre
            // re-resolver ao vivo.
            if (tipoPedido) doBerco.tipos = { ...(doBerco.tipos || {}), [lado]: tipoPedido };
          }
