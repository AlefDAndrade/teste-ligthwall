...
module.exports = function criarRotasUsuarios({ fs, path, PRIVATE_DIR, auth, sessao, sessaoUsuario, perfis, perfisCustomizados, perfisFixosOverrides, itensPermissao }) {
  ...
  // ─── Perfis fixos + customizados, mesclados ────────────────────────────
  // Perfis customizados (ver lib/perfis-customizados.js — criados pelo
  // Administrador em Configurações → Usuários → "+ Criar novo tipo de
  // perfil") somam-se aos 6 perfis fixos de lib/perfis.js — nunca
  // substituem, nunca reinterpretam os fixos. Todo o resto do sistema
  // (login, cadastro, GET /perfis, podeControlarOperacao em server.js)
  // passa a considerar "perfil válido" = fixo OU customizado.

  function todosPerfisCadastraveis() {
    return [...perfis.PERFIS_CADASTRAVEIS, ...perfisCustomizados.listar().map(p => p.id)];
  }

  function perfilExiste(perfilId) {
    return perfis.PERFIS_CADASTRAVEIS.includes(perfilId) || perfisCustomizados.ehPerfilCustomizado(perfilId);
  }

  // Devolve o mapa item-a-item efetivo de um perfil FIXO — o override
  // salvo (ver lib/perfis-fixos-overrides.js — voltou, ver conversa que
  // motivou a mudança), ou `null` se nunca foi customizado (comportamento
  // hardcoded padrão de lib/perfis.js continua valendo).
  function overrideDoPerfilFixo(perfilId) {
    return perfisFixosOverrides.obter(perfilId);
  }

  function paginasPorPerfilMescladas() {
    const mapa = { ...perfis.PAGINAS_POR_PERFIL };
    for (const perfilId of perfis.PERFIS_CADASTRAVEIS) {
      const override = overrideDoPerfilFixo(perfilId);
      if (override) mapa[perfilId] = perfisCustomizados.paginasPermitidas({ permissoes: override });
    }
    for (const customizado of perfisCustomizados.listar()) {
      mapa[customizado.id] = perfisCustomizados.paginasPermitidas(customizado);
    }
    return mapa;
  }

  function areasEdicaoPorPerfilMescladas() {
    const mapa = { ...perfis.AREAS_EDICAO_POR_PERFIL };
    for (const perfilId of perfis.PERFIS_CADASTRAVEIS) {
      const override = overrideDoPerfilFixo(perfilId);
      if (override) mapa[perfilId] = perfisCustomizados.areasDeEdicao({ permissoes: override });
    }
    for (const customizado of perfisCustomizados.listar()) {
      mapa[customizado.id] = perfisCustomizados.areasDeEdicao(customizado);
    }
    return mapa;
  }

  // Itens "Outros" (tipo 'acao': Importar Documentos, Exportações
  // Interativas/Excel, Edição dos Dados, Backup e Restauração) que cada
  // perfil tem marcado "Acesso Total" — mesmo padrão de
  // areasEdicaoPorPerfilMescladas() (acima), só que por ITEM em vez de
  // por área (ver podeUsarItem, lib/permissoes-area.js, que faz a mesma
  // conta do lado do servidor pra validar de verdade; isto aqui é só o
  // que o FRONT usa pra mostrar/esconder os botões correspondentes,
  // ver _perfilTemAcao(), app-core.js). "Administrativo" (perfil fixo
  // "Administrador" cadastrado) sempre tem todos — é irrestrito por
  // definição, igual `ehPerfilDeAdmin()` já trata em lib/perfis.js.
  function itensAcaoPorPerfilMesclados() {
    const itensAcao = itensPermissao.CATALOGO.filter(item => item.tipo === 'acao').map(item => item.id);
    const mapa = {};
    for (const perfilId of perfis.PERFIS_CADASTRAVEIS) {
      if (perfilId === 'Administrativo') { mapa[perfilId] = itensAcao; continue; }
      const override = overrideDoPerfilFixo(perfilId);
      mapa[perfilId] = override
        ? itensAcao.filter(id => perfisCustomizados.nivelDoItem({ permissoes: override }, id) === 'total')
        : []; // perfil fixo sem override nunca teve estes itens (só Administrativo tinha, antes desta mudança)
    }
    for (const customizado of perfisCustomizados.listar()) {
      mapa[customizado.id] = itensAcao.filter(id => perfisCustomizados.nivelDoItem(customizado, id) === 'total');
    }
    return mapa;
  }

  function rotulosPorPerfilMesclados() {
    const mapa = { ...perfis.ROTULO_POR_PERFIL };
    for (const customizado of perfisCustomizados.listar()) {
      mapa[customizado.id] = customizado.nome;
    }
    return mapa;
  }

  function perfisComControleDeOperacaoMesclados() {
    const fixos = perfis.PERFIS_CADASTRAVEIS.filter(perfilId => {
      const override = overrideDoPerfilFixo(perfilId);
      return override
        ? perfisCustomizados.podeEditar({ permissoes: override }, 'injetora')
        : perfis.PERFIS_COM_CONTROLE_DE_OPERACAO.includes(perfilId);
    });
    const customizadosComInjetora = perfisCustomizados.listar()
      .filter(p => perfisCustomizados.podeEditar(p, 'injetora'))
      .map(p => p.id);
    return [...fixos, ...customizadosComInjetora];
  }
  ...
  return function tentar(req, res, urlPath, queryParams) {
    ...
    if (req.method === 'GET' && urlPath === '/perfis') {
      ...
      res.end(JSON.stringify({
        ok: true,
        paginasPorPerfil: paginasPorPerfilMescladas(),
        // Modelo novo (ver lib/perfis.js): além de "quais páginas cada
        // perfil VÊ" (acima, hoje quase tudo pra todo mundo), o front
        // também precisa de "quais áreas cada perfil EDITA" — pra
        // esconder/desabilitar formulários e botões de registro nas
        // páginas em que o perfil só visualiza (a validação de verdade
        // continua sendo a do servidor, rota a rota).
        areasEdicaoPorPerfil: areasEdicaoPorPerfilMescladas(),
        // Ver comentário de itensAcaoPorPerfilMesclados(), acima.
        itensAcaoPorPerfil: itensAcaoPorPerfilMesclados(),
        rotulosPorPerfil: rotulosPorPerfilMesclados(),
        perfisCadastraveis: todosPerfisCadastraveis(),
        perfisComControleDeOperacao: perfisComControleDeOperacaoMesclados(),
      }));
      return true;
    }

    // ── GET /permissoes-perfil-fixo?perfil=OperadorInjetora — engrenagem
    // ⚙️ ao lado do campo "Perfil" em Configurações → Usuários (voltou —
    // ver conversa que motivou a mudança). Devolve o mapa item-a-item
    // ATUAL (override salvo, ou o padrão hardcoded traduzido pro catálogo
    // — ver lib/perfis.js, permissoesPadraoDoPerfilFixo) pra pré-marcar a
    // mesma tela de "Acesso Total / Apenas Visualizar / Ocultar" que já
    // existe pra perfis customizados. Leitura pública (mesmo raciocínio
    // de GET /perfis e GET /perfis-customizados — não é segredo, só
    // descreve o que o perfil já pode fazer).
    if (req.method === 'GET' && urlPath === '/permissoes-perfil-fixo') {
      const perfilId = queryParams.get('perfil') || '';
      if (!perfis.PERFIS_CADASTRAVEIS.includes(perfilId)) {
        res.writeHead(400, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ ok: false, erro: `"${perfilId}" não é um dos 6 perfis fixos do sistema.` }));
        return true;
      }
      const override = overrideDoPerfilFixo(perfilId);
      res.writeHead(200, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify({
        ok: true,
        perfil: perfilId,
        rotulo: perfis.ROTULO_POR_PERFIL[perfilId],
        permissoes: override || perfis.permissoesPadraoDoPerfilFixo(perfilId),
        temOverride: !!override,
      }));
      return true;
    }

    // ── POST /salvar-permissoes-perfil-fixo { perfil, permissoes } —
    // salva o override item-a-item de um perfil FIXO (ver
    // lib/perfis-fixos-overrides.js). A partir daqui, o mapa salvo manda
    // pra ESTE perfil — ver podeEditarArea()/podeControlarOperacao(),
    // server.js. Mesma exigência de sessão de administrador (master OU
    // Administrativo — `sessao` aqui já vem como o wrapper `sessaoOuAdmin`
    // de server.js) das outras rotas de gerenciar perfil, nunca menos —
    // pedido explícito do usuário ("pra salvar isso tem que ser adm e ter
    // senha de adm").
    if (req.method === 'POST' && urlPath === '/salvar-permissoes-perfil-fixo') {
      ...
      req.on('end', () => {
        try {
          ...
          if (!perfis.PERFIS_CADASTRAVEIS.includes(perfilId)) {
            throw new Error(`"${perfilId}" não é um dos 6 perfis fixos do sistema.`);
          }
          const salvo = perfisFixosOverrides.salvar(perfilId, payload.permissoes);
          res.writeHead(200, { 'Content-Type': 'application/json' });
          res.end(JSON.stringify({ ok: true, perfil: perfilId, permissoes: salvo }));
        } catch (e) {
          res.writeHead(400, { 'Content-Type': 'application/json' });
    ...
    // ── POST /restaurar-permissoes-perfil-fixo { perfil } — remove o
    // override salvo; o perfil volta ao comportamento hardcoded padrão de
    // lib/perfis.js. Mesma exigência de sessão de administrador da rota
    // de salvar, acima.
    if (req.method === 'POST' && urlPath === '/restaurar-permissoes-perfil-fixo') {
      if (!sessao.requestTemSessaoValida(req)) { semSessaoAdmin(res); return true; }
      let body = '';
      req.on('data', chunk => body += chunk);
      req.on('end', () => {
        try {
          ...
          perfisFixosOverrides.remover(perfilId);
          res.writeHead(200, { 'Content-Type': 'application/json' });
          res.end(JSON.stringify({ ok: true, perfil: perfilId, permissoes: perfis.permissoesPadraoDoPerfilFixo(perfilId) }));
        } catch (e) {
          res.writeHead(400, { 'Content-Type': 'application/json' });
          res.end(JSON.stringify({ ok: false, erro: e.message }));
        }
    ...
    if (req.method === 'GET' && urlPath === '/usuarios') {
      // Inclui podeIniciarOperacao (além de id/nomeUsuario/perfil) — sem
      // isso, a tela de Configurações → Usuários nunca sabia, depois de
      // recarregar a lista, se um usuário já cadastrado tinha ou não essa
      // marcação (ficava sempre `undefined`/false na cache do front,
      // mesmo pra quem tinha sido salvo com `true`). Ver
      // cfgRenderUsuarios/cfgToggleIniciarOperacao, public/js/app-core.js.
      const lista = lerUsuarios().map(u => ({
        id: u.id,
        nomeUsuario: u.nomeUsuario,
        perfil: u.perfil,
        podeIniciarOperacao: !!u.podeIniciarOperacao,
      }));
      res.writeHead(200, { 'Content-Type': 'application/json' });
    ...
    if (req.method === 'POST' && urlPath === '/login-usuario') {
      ...
      req.on('end', () => {
        try {
          ...
          // Perfil descontinuado (cadastro anterior à mudança dos 6 perfis
          // novos — ex: "Analista", "Operador", "Qualidade" antigos): não
          // emite sessão nenhuma, porque nenhuma checagem de permissão
          // saberia o que fazer com ele. Decisão registrada na mudança:
          // esses usuários são excluídos/recadastrados manualmente pelo
          // Administrador (Configurações → Usuários), não migrados.
          if (!perfilExiste(usuario.perfil)) {
            throw new Error(`O perfil "${usuario.perfil}" foi descontinuado. Peça a um Administrador pra recadastrar seu usuário com um dos perfis novos.`);
          }
          const dadosSessao = {
      ...
      req.on('end', () => {
        try {
          ...
          const nova = entrada.map((u, idx) => {
            ...
            // Só exige perfil VÁLIDO se o usuário é novo, ou se o perfil
            // dele está de fato mudando nesta operação — usuário
            // existente cujo perfil não está sendo tocado passa direto,
            // mesmo que esse perfil tenha ficado órfão nesse meio tempo
            // (ex: perfil customizado excluído, ou usuarios.json
            // restaurado de um Backup Geral mais antigo sem o
            // perfis-customizados.json correspondente — ver
            // lib/rotas/backup.js). Sem isso, um ÚNICO usuário com perfil
            // órfão travava CADASTRAR ou REMOVER qualquer outro usuário —
            // /salvar-usuarios sempre reenvia a lista inteira, então a
            // validação batia em todo mundo, não só em quem estava sendo
            // alterado. Editar/recadastrar ESSE usuário específico (perfil
            // realmente mudando) continua exigindo um perfil válido,
            // então não dá pra criar um órfão novo por essa rota.
            const perfilMudou = !existente || existente.perfil !== u.perfil;
            if (perfilMudou && !perfilExiste(u.perfil)) {
              throw new Error(`Usuário "${nomeUsuario}": perfil "${u.perfil}" inválido. Precisa ser um de: ${todosPerfisCadastraveis().join(', ')}.`);
            }
            ...
            // "podeIniciarOperacao" só é significativo pra perfis que têm
            // a área 'injetora' de edição SEM serem administradores (ver
            // PERFIS_COM_CONTROLE_DE_OPERACAO, lib/perfis.js) — pra
            // qualquer outro, força false independente do que veio no
            // payload: ou a marcação não significaria nada (o perfil não
            // registra operação de jeito nenhum), ou é redundante (o
            // perfil Administrativo sempre pode, irrestrito).
            const podeIniciarOperacao = perfisComControleDeOperacaoMesclados().includes(u.perfil)
