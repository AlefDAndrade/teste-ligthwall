  ...
  return function tentar(req, res, urlPath) {
    ...
    if (req.method === 'GET' && urlPath === '/perfis') {
      ...
      res.end(JSON.stringify({
        ...
        areasEdicaoPorPerfil: perfis.AREAS_EDICAO_POR_PERFIL,
        rotulosPorPerfil: perfis.ROTULO_POR_PERFIL,
        perfisCadastraveis: perfis.PERFIS_CADASTRAVEIS,
        perfisComControleDeOperacao: perfis.PERFIS_COM_CONTROLE_DE_OPERACAO,
      }));
      return true;
    }
    ...
    if (req.method === 'POST' && urlPath === '/login-usuario') {
      ...
      req.on('end', () => {
        try {
          ...
          if (!perfis.PERFIS_CADASTRAVEIS.includes(usuario.perfil)) {
      ...
      req.on('end', () => {
        try {
          ...
          const nova = entrada.map((u, idx) => {
            ...
            const podeIniciarOperacao = perfis.PERFIS_COM_CONTROLE_DE_OPERACAO.includes(u.perfil)
