    ...
    if (req.method === 'POST' && urlPath === '/manutencao/corretiva') {
      lerCorpoJSON(req).then(chamado => {
        try {
          ...
          // Rastreia se alguma notificação ESPECÍFICA (das duas acima)
          // disparou pra ESTE salvamento — usado logo abaixo pra decidir
          // se dispara a genérica de "atualização" (ver comentário lá).
          let notificacaoEspecificaDisparada = false;
          ...
          if (existente && typeof notificarPedidoPeca === 'function') {
            ...
            if (salvo.situacao === 'Em Manutencao' && agoraEstaAguardandoPecas && !jaEstavaAguardandoPecas) {
              ...
              notificacaoEspecificaDisparada = true;
            }
          }
          ...
          // Notifica push GENÉRICO de "atualização de etiqueta" — pedido
          // do usuário: "notificar todas as atualizações que forem
          // feitas em uma etiqueta". Só em UPDATES de um chamado JÁ
          // EXISTENTE (`existente`, nunca na criação — essa já tem seu
          // próprio aviso de "abertura", acima) E só quando NENHUMA das
          // notificações específicas de cima disparou pra este mesmo
          // salvamento (`!notificacaoEspecificaDisparada`) — evita
          // duplicar aviso quando o evento já é coberto por um alerta
          // mais específico (ver lib/notificacoes-push.js). Cobre, por
          // exemplo: editar Abertura/Detalhes/Execução/Acompanhamento,
          // ou fechar a etiqueta (etiquetaFechada/Concluído).
          if (existente && !notificacaoEspecificaDisparada && typeof notificarAtualizacaoChamadoCorretivo === 'function') {
            notificarAtualizacaoChamadoCorretivo(salvo, nomeDeQuemAceita(req));
          }
    ...
    if (req.method === 'POST' && urlPath === '/manutencao/programada') {
      ...
      lerCorpoJSON(req).then(agendamento => {
        try {
          ...
          } else if (existente && typeof notificarAtualizacaoManutencaoProgramada === 'function') {
            // Notifica push GENÉRICO de "atualização de etiqueta" — pedido
            // do usuário: mesmo raciocínio do chamado corretivo, acima,
            // aplicado aqui pra agendamentos JÁ EXISTENTES sendo
            // atualizados (aprovar/reprovar/executar/editar) — hoje só a
            // CRIAÇÃO tinha aviso (ver notificarManutencaoProgramada,
            // if acima).
            notificarAtualizacaoManutencaoProgramada(salvo, nomeDeQuemAceita(req));
          }
        } catch (e) {
          responderErro(res, 400, e.message);
        }
