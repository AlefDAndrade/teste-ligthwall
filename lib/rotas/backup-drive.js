...
// módulos de lib/rotas/ (ver lib/rotas/backup.js pro
...
// backend"): só `GET /status` (leitura, mostra se está conectado — sem
// expor token) trocou `sessao: sessaoOuAdmin` por
// `podeUsarItem(req, 'backup-restauracao')` (lib/permissoes-area.js),
// liberando também um perfil customizado que marcou "Backup e
// Restauração: Acesso Total". As outras 3 (autorizar/toggle/desconectar)
// CONTINUAM exigindo sessaoOuAdmin — conectar/reconfigurar/desconectar
// uma conta do Google é uma mudança de configuração mais sensível que só
// baixar um backup, então fica de fora deste item de propósito (2 delas
// também exigem a senha reverificada, ver comentário de `autorizar`
// acima).
...
module.exports = function criarRotasBackupDrive({ sessao, podeUsarItem, negarAcesso, auth, googleDrive, backupDriveJson }) {
  ...
  function tentar(req, res, urlPath) {
    ...
    if (req.method === 'GET' && urlPath === '/backup-drive/status') {
      if (!podeUsarItem(req, 'backup-restauracao')) { negarAcesso(res, 'acessar backups'); return true; }
