    ...
    if (req.method === 'POST' && urlPath === '/mesclar-backup-dados') {
      ...
      req.on('end', () => {
        try {
          ...
          if (!presentes.length) {
            throw new Error('Nenhum arquivo mesclável encontrado no backup (historico.json, relatorio_injecao.json, ajustes_tracos.json, paradas.json, tracos_descartados.json, bercos_visuais.json, avaliacoes_qualidade.json, operacoes_avaliadas.json ou relatorio_edicoes.json).');
          }
          ...
          if (filtroDataInicio || filtroDataFim) {
            ...
            // Dos domínios satélite adicionados nesta mudança, só
            // avaliacoes_qualidade tem campo de data PRÓPRIO —
            // bercos_visuais/operacoes_avaliadas/edicoes_traco são
            // amarrados a uma operação/traço (já respeitam o filtro
            // indiretamente, via checagem de "o dono existe", mais abaixo).
  ...
  function _restaurarArquivosDeDados(arquivos, prefixoDirSeguranca) {
    ...
    const nomesArquivo = esperados.filter(n =>
      ...
      !['historico.json', 'historico_edicoes.json', 'paradas.json', 'sobra.json', 'contador_tracos.json',
        ...
        'bercos_visuais.json', 'avaliacoes_qualidade.json', 'operacoes_avaliadas.json',
        'tracos_descartados.json'].includes(n));
