        ...
        try {
          ...
          try {
            ...
            if (erroSecurity instanceof auth.ErroSecurityIndisponivel) {
              console.error('[auth] /mesclar-backup-dados: security.json indisponível —', erroSecurity.message);
      ...
      req.on('end', () => {
        try {
          ...
          try {
            ...
            if (erroSecurity instanceof auth.ErroSecurityIndisponivel) {
              console.error('[auth] /restaurar-backup-dados: security.json indisponível —', erroSecurity.message);
              res.writeHead(503, { 'Content-Type': 'application/json' });
              res.end(JSON.stringify({ ok: false, erro: 'Autenticação temporariamente indisponível. Contate o suporte técnico.' }));
              return;
            }
      ...
      req.on('end', () => {
        ...
        try {
          ...
          try {
            ...
            if (erroSecurity instanceof auth.ErroSecurityIndisponivel) {
              console.error('[auth] /restaurar-backup-geral: security.json indisponível —', erroSecurity.message);
              res.writeHead(503, { 'Content-Type': 'application/json' });
              res.end(JSON.stringify({ ok: false, erro: 'Autenticação temporariamente indisponível. Contate o suporte técnico.' }));
              return;
            }
