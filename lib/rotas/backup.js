...
module.exports = function criarRotasBackup({
  ...
  //     usuarios.json, operadores.json) — some do Backup de Dados, mas
  //     ENTRA no Backup Geral, somado à lista acima (ver
  //     VALIDADORES_BACKUP_GERAL, mais abaixo).
  ...
  // senhas em hash, perfis, atalhos; operadores.json — Identidade Leve).
  ...
  function tentar(req, res, urlPath, queryParams) {
    ...
    // usuarios.json, operadores.json (esses 4 são exclusivos do Backup
    // Geral, ver GET /backup-geral, logo abaixo).
  ...
  // config.json/security.json/usuarios.json/operadores.json (ver
  ...
  // arquivo): estes 4 são leitura/escrita direta de arquivo, sem SQL —
  // bem mais simples. security.json/usuarios.json/operadores.json são
  // OPCIONAIS e PRESERVADOS se ausentes do backup (ver conversa que
  // motivou isso: restaurar um backup do sistema ANTIGO, que nunca teve
  // esses 3 arquivos, não pode apagar os usuários já cadastrados no
  // sistema atual — image um Admin restaurando um backup de dados
  // antigos só pra recuperar histórico de produção, sem querer mexer em
  // quem tem acesso ao sistema). config.json continua OBRIGATÓRIO (não
  // existir de jeito nenhum no sistema, nem antigo nem novo, indicaria
  // um backup malformado).
  function _restaurarArquivosExtraDoGeral(arquivos, dirSeguranca) {
    const OPCIONAIS_E_PRESERVADOS = ['security.json', 'usuarios.json', 'operadores.json'];
    ...
    // Backup de segurança dos 4 arquivos extra — mesmo dirSeguranca já
    ...
    if (req.method === 'POST' && urlPath === '/restaurar-backup-geral') {
      ...
      req.on('end', () => {
        ...
        try {
          ...
          // pelos 4 arquivos extra logo abaixo.
          const { presentes: presentesDados, dirSeguranca } = _restaurarArquivosDeDados(arquivos, 'pre-restore-geral');

          // Os 4 arquivos extra do Geral (config/identidade) — ver
          // _restaurarArquivosExtraDoGeral: security.json/usuarios.json/
          // operadores.json são OPCIONAIS e PRESERVADOS se ausentes (ex:
          // restaurando um backup do sistema ANTIGO, que nunca teve esses
          // arquivos — não pode apagar os usuários já cadastrados no
          // sistema atual).
