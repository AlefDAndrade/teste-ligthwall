...
module.exports = function criarRotasBackup({
  ...
  const VALIDADORES_DADOS_PRODUCAO = {
    ...
    'manutencao_estoque.json':       v => Array.isArray(v),
    'manutencao_movimentacoes.json': v => Array.isArray(v),
  };
  ...
  const DEFAULT_SE_VAZIO_BACKUP_DADOS = {
    ...
    'manutencao_estoque.json': [],
    'manutencao_movimentacoes.json': [],
  };
  ...
  async function _gerarZipComArquivos(listaDeArquivos) {
    ...
    listaDeArquivos.forEach(nome => {
      try {
        ...
        } else if (nome === 'manutencao_estoque.json') {
          zip.file(nome, JSON.stringify(db.listarManutencaoEstoque(), null, 2));
        } else if (nome === 'manutencao_movimentacoes.json') {
          zip.file(nome, JSON.stringify(db.listarManutencaoMovimentacoes(), null, 2));
        } else {
  ...
  function _restaurarArquivosDeDados(arquivos, prefixoDirSeguranca) {
    ...
    const OPCIONAIS_BACKUP_DADOS = [
      ...
      'manutencao_corretiva.json', 'manutencao_programada.json', 'manutencao_estoque.json', 'manutencao_movimentacoes.json',
    ];
    ...
    for (const nome of esperados) {
      try {
        if (nome === 'historico.json') {
          ...
          fs.writeFileSync(path.join(dirSeguranca, nome), JSON.stringify(db.listarManutencaoEstoque(), null, 2), 'utf8');
        } else if (nome === 'manutencao_movimentacoes.json') {
          fs.writeFileSync(path.join(dirSeguranca, nome), JSON.stringify(db.listarManutencaoMovimentacoes(), null, 2), 'utf8');
        } else {
    ...
    const nomesArquivo = esperados.filter(n =>
      ...
      !['historico.json', 'historico_edicoes.json', 'paradas.json', 'sobra.json', 'contador_tracos.json',
        ...
        'manutencao_corretiva.json', 'manutencao_programada.json', 'manutencao_estoque.json', 'manutencao_movimentacoes.json'].includes(n));
    ...
    // Estoque ANTES de movimentações, de propósito — foreign_keys está
    // ATIVADO neste banco (manutencao_movimentacoes.peca_id REFERENCES
    // manutencao_estoque(id)); substituirManutencaoEstoque() já cuida de
    // apagar movimentações antigas antes de apagar o estoque (ver
    // comentário na função, db.js) — mas a ORDEM de chamada aqui ainda
    // importa: se movimentações fosse restaurada primeiro, o INSERT dela
    // falharia porque o estoque novo (que ela referencia) ainda não
    // existiria.
    if (presentes.includes('manutencao_estoque.json')) {
      const novoEstoque = JSON.parse(textosValidados['manutencao_estoque.json']);
      db.transaction(() => db.substituirManutencaoEstoque(novoEstoque))();
    }
    if (presentes.includes('manutencao_movimentacoes.json')) {
      const novasMovimentacoes = JSON.parse(textosValidados['manutencao_movimentacoes.json']);
      db.transaction(() => db.substituirManutencaoMovimentacoes(novasMovimentacoes))();
    }
