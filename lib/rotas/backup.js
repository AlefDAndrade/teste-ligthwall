    ...
    if (req.method === 'POST' && urlPath === '/mesclar-backup-dados') {
      ...
      req.on('end', () => {
        try {
          ...
          const MESCLAVEIS = ['historico.json', 'historico_edicoes.json', 'relatorio_injecao.json', 'ajustes_tracos.json', 'paradas.json', 'tracos_descartados.json'];
          const presentes = MESCLAVEIS.filter(nome => typeof arquivos[nome] === 'string');
          if (!presentes.length) {
            throw new Error('Nenhum arquivo mesclável encontrado no backup (historico.json, relatorio_injecao.json, ajustes_tracos.json, paradas.json ou tracos_descartados.json).');
          }
