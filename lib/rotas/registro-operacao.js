...
module.exports = function criarRotasRegistroOperacao({
  ...
  return function tentar(req, res, urlPath, queryParams) {
    ...
    if (req.method === 'POST' && urlPath === '/registrar-operacao') {
      ...
      req.on('end', () => {
        if (!modoTeste && !podeControlarOperacao(req)) { negarControleDeOperacao(res); return; }
        try {
          ...
          if (modoTeste) {
            ...
            let historico = [];
            // Arquivo pode não existir ainda (primeira operação em modo de
            // teste) ou estar corrompido — segue com histórico vazio; a
            // operação atual é gravada de qualquer forma logo abaixo.
            ...
            // Berços Visuais — 1 linha por berço desta operação. Sempre a
            // capacidade nominal da bateria (mesma prioridade já usada em
            // "Bateria Atual" — ver bateria-atual.js, _baCapacidadeConfigurada).
            // Estados: parte do que já foi marcado ao vivo (baixou/vazou —
            // ver GET/POST /bercos-andamento) em vez de nascer tudo 'okay' à
            // toa; e reseta o snapshot ao vivo logo em seguida — essa
            // operação virou histórico agora, o snapshot é só pra enquanto
            // ela está em andamento. Berço que fisicamente não foi usado se
            // marca DEPOIS, individualmente, como 🚫 Não Enchido (não muda
            // a quantidade de linhas criada aqui).
        ...
        try {
          ...
          db.transaction(() => {
            dadosRecebidos.forEach(novoTraco => {
              ...
              // Insumos CUSTOM (Fase 3, corrigido depois da Fase 6 — ver
              // PLANO-insumos-dinamicos-receitas.md) — roda em QUALQUER
              // caso (novo OU já existente), diferente dos 5 Padrão
              // acima: o botão "+" deixa adicionar um insumo Custom a um
              // traço já em andamento, inclusive depois que ele já foi
              // submetido uma vez (ex: mesmo traço usado em 2+ baterias
              // da mesma operação). salvarInsumosCustomDoTraco usa
              // INSERT OR IGNORE (ver lib/db/tracos.js) — quem já tem
              // valor gravado pra aquele insumo simplesmente é ignorado,
              // preservando a regra de nunca reescrever a receita original.
