...
module.exports = function criarRotasAutenticacao({ fs, path, DB_DIR, SECURITY_PATH, auth, sessao }) {
  ...
  return function tentar(req, res, urlPath) {
        ...
        try {
          ...
          try {
            ...
            if (erroSecurity instanceof auth.ErroSecurityIndisponivel) {
              logger.error('auth', '/verificar-senha: security.json indisponível', { erro: erroSecurity.message });
          ...
          const ok = auth.validarSegredo(senha, security.passwordHash, 'passwordHash');
      ...
      req.on('end', () => {
        try {
          ...
          try {
            ...
            if (erroSecurity instanceof auth.ErroSecurityIndisponivel) {
              logger.error('auth', '/verificar-recovery: security.json indisponível', { erro: erroSecurity.message });
              res.writeHead(503, { 'Content-Type': 'application/json' });
              res.end(JSON.stringify({ ok: false, erro: 'Autenticação temporariamente indisponível. Contate o suporte técnico.' }));
              return;
            }
    ...
    // montagem se o front esquecesse de incluir algum bloco (já
    // aconteceu antes com outros campos, ver comentário em
    ...
    if (req.method === 'GET' && urlPath === '/db/security.json') {
      ...
      try {
        ...
        JSON.parse(conteudo); // valida que não está corrompido antes de devolver
        res.writeHead(200, { 'Content-Type': 'application/json' });
        res.end(conteudo);
      } catch (erro) {
        // Sessão já validada acima (linha anterior) — quem chegou aqui já
        // provou saber a senha/chave de recuperação atual. Mesmo assim,
        // NÃO inventa um hash: se o arquivo sumiu/corrompeu depois da
        // sessão ter sido criada, é um problema de infraestrutura, não
        // uma senha errada — 503, igual às outras rotas de auth.
        logger.error('auth', 'GET /db/security.json: arquivo indisponível', { erro: erro.message });
        res.writeHead(503, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ ok: false, erro: 'Arquivo de segurança indisponível. Contate o suporte técnico.' }));
      }
      return true;
    }
