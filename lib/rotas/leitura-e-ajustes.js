...
module.exports = function criarRotasLeituraEAjustes({ fs, path, db, DB_DIR, dirParaModoTeste, broadcastLeituraAutomatica, agoraBrasiliaISOServer }) {
      ...
      req.on('end', () => {
        try {
          ...
          if (modoTeste) {
            ...
            let ajustesTracos = [];
            // Arquivo pode não existir ainda (primeiro ajuste em modo de teste)
            // ou estar corrompido — segue com lista vazia em vez de quebrar o registro.
            ...
            entrada['ajuste_' + proximoNumero] = { ...ajuste, registrado_em: agoraBrasiliaISOServer() };
          ...
          const infoAjuste = db.prepare(db.SQL_INSERIR_AJUSTE).run({
            id_traco,
            ordem: proximaOrdem,
            ...
            registrado_em: agoraBrasiliaISOServer(),
          });
          // Insumos CUSTOM deste ajuste (Fase 3, ver
          // PLANO-insumos-dinamicos-receitas.md) — "vale pro traço inteiro"
          // (decisão tomada na conversa) é responsabilidade do FRONT exigir
          // o campo de novo a cada ajuste (Fase 5); aqui só grava o que
          // vier.
          db.salvarInsumosCustomDoAjuste(infoAjuste.lastInsertRowid, ajuste.insumos_custom);
