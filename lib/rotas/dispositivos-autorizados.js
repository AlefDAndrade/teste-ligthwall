    ...
    if (req.method === 'POST' && urlPath === '/autorizar-dispositivo') {
      ...
      req.on('end', () => {
        try {
          ...
          // IP de quem está SUBMETENDO o formulário de autorização — na
          // prática, quase sempre o próprio dispositivo sendo autorizado
          // (Configurações → Dispositivos Autorizados aberto nele mesmo).
          // Guardado só pra alimentar o auto-reconhecimento por IP (ver
          // dispositivoAutorizado(), server.js) — se este deviceId for
          // perdido depois (cookie/localStorage limpos), um request vindo
          // do MESMO IP religa sozinho, sem o Administrador precisar
          // reautorizar manualmente. Puramente informativo/best-effort:
          // não é usado como trava por si só.
