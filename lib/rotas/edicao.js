...
module.exports = function criarRotasEdicao({ db, podeEditarArea, negarEdicao, numOuNulo, agoraBrasiliaISOServer }) {
  ...
  // Estados válidos de UM lado de UM berço em bercos_visuais — mesmo
  // vocabulário de sempre (ver bateria-atual.js/relatorio-bercos.js):
  // 'okay' (nada marcado), 'baixou' (vazou/baixou) ou 'nao_enchido'
  // (berço nem chegou a ser usado — o painel correspondente nunca
  // existiu). Validado aqui (servidor) antes de gravar — nunca confia
  // só na validação do navegador (mesmo espírito do resto do arquivo).
  const ESTADOS_BERCO_VISUAL_VALIDOS = new Set(['okay', 'baixou', 'nao_enchido']);
  function _validarBercosVisuais(lista) {
    if (!Array.isArray(lista)) throw new Error('Formato inválido de berços visuais.');
    lista.forEach(item => {
      if (!item || typeof item.berco !== 'string' || !/^B\d+$/.test(item.berco)) {
        throw new Error('Berço inválido em berços visuais.');
      }
      if (item.estado_esquerda !== undefined && item.estado_esquerda !== null && !ESTADOS_BERCO_VISUAL_VALIDOS.has(item.estado_esquerda)) {
        throw new Error(`Estado inválido (esquerda) no berço ${item.berco}.`);
      }
      if (item.estado_direita !== undefined && item.estado_direita !== null && !ESTADOS_BERCO_VISUAL_VALIDOS.has(item.estado_direita)) {
        throw new Error(`Estado inválido (direita) no berço ${item.berco}.`);
      }
    });
  }
  ...
  return function tentar(req, res, urlPath) {
    ...
    if (req.method === 'POST' && urlPath === '/editar-operacao') {
      ...
      req.on('end', () => {
        try {
          ...
          const { id, novosValores, diff, bercosVisuais } = payload;
          ...
          // bercosVisuais — OPCIONAL, à parte de novosValores/CAMPOS_PROTEGIDOS
          // porque não é coluna de "operacoes": vive na tabela bercos_visuais
          // (1 linha por operação, ver db.js). Só vem preenchido quando o
          // admin efetivamente abriu e mexeu no editor "🔲 Berços — Não
          // Enchido/Vazou" (ver _eoAbrirBercosVisuais, app-core.js) — ausente
          // (undefined) significa "não mexer nisso", nunca apaga o que já
          // existia.
          if (bercosVisuais !== undefined) _validarBercosVisuais(bercosVisuais);
          ...
          // UPDATE de "operacoes" + (se veio) upsert de bercos_visuais na
          // MESMA transação: as duas fazem parte de uma única "Salvar
          // Alterações" na tela (um clique, uma confirmação, um log de
          // auditoria) — nunca deveria ficar um salvo e o outro não.
          const executarEdicao = db.transaction(() => {
            db.prepare(`
            ...
            if (bercosVisuais !== undefined) {
              db.prepare(`
                INSERT INTO bercos_visuais (id_operacao, bercos, atualizado_em)
                VALUES (@id_operacao, @bercos, @atualizado_em)
                ON CONFLICT(id_operacao) DO UPDATE SET bercos = excluded.bercos, atualizado_em = excluded.atualizado_em
              `).run({ id_operacao: id, bercos: JSON.stringify(bercosVisuais), atualizado_em: new Date().toISOString() });
            }
          });
          executarEdicao();
      ...
      req.on('end', () => {
        try {
          ...
          db.transaction(() => {
            ...
            // ajuste_insumos (Fase 3, insumos CUSTOM) tem que ser apagada
            // ANTES de "ajustes" — depende do id_ajuste (autoincrement) que
            // está prestes a sumir junto com as linhas antigas.
            db.prepare('DELETE FROM ajuste_insumos WHERE id_ajuste IN (SELECT id FROM ajustes WHERE id_traco = ?)').run(id_traco);
            ...
            ajustes.forEach((a, i) => {
              ...
              db.salvarInsumosCustomDoAjuste(infoAjuste.lastInsertRowid, a.insumos_custom);
            });

            // Insumos CUSTOM "originais" (Fase 3) — mesma ideia de
            // substituir tudo: apaga o que já existia pra este traço e
            // regrava a partir de originais.insumos_custom (se vier).
            db.prepare('DELETE FROM traco_insumos WHERE id_traco = ?').run(id_traco);
            db.salvarInsumosCustomDoTraco(id_traco, originais.insumos_custom);
    ...
    // ── GET /bercos-visuais-operacao/:id: berços (okay/baixou/nao_enchido,
    // 2 lados cada) já marcados nesta operação — usado pelo editor visual
    // "🔲 Berços — Não Enchido/Vazou" dentro de Editar Operação (ver
    // _eoAbrirBercosVisuais, app-core.js), pra ter o que desenhar/editar
    // antes de qualquer "Salvar Alterações". Sem linha ainda em
    // bercos_visuais (operação antiga, de antes desta tabela existir, ou
    // nunca teve nada marcado) -> lista vazia, o front nasce tudo 'okay'.
    if (req.method === 'GET' && urlPath.startsWith('/bercos-visuais-operacao/')) {
      if (!podeEditarArea(req, 'injetora')) { negarEdicao(res, 'os berços visuais da operação'); return true; }
      const idOperacao = decodeURIComponent(urlPath.slice('/bercos-visuais-operacao/'.length));
      const row = db.prepare('SELECT bercos FROM bercos_visuais WHERE id_operacao = ?').get(idOperacao);
      res.writeHead(200, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify({ ok: true, bercos: row ? JSON.parse(row.bercos) : [] }));
      return true;
    }
