...
// backend"): antes exigia sessaoOuAdmin (Admin Master ou perfil fixo
// "Administrador") — agora usa `podeUsarItem(req, 'importar-documentos')`
// (lib/permissoes-area.js), que TAMBÉM libera um perfil customizado (ou
// perfil fixo com override) que marcou "Importar Documentos: Acesso
// Total" no catálogo, sem conceder nenhuma outra permissão administrativa
// — Admin Master/"Administrador" continuam irrestritos, comportamento
// idêntico ao de antes pra quem não mexeu em nada.

module.exports = function criarRotasImportacao({ db, podeUsarItem, negarAcesso, numOuNulo }) {
  ...
  return function tentar(req, res, urlPath) {
    ...
    // Administrador. Agora exige permissão de verdade (ver comentário de
    // PERMISSÃO, acima).
    if (req.method === 'POST' && urlPath === '/importar-relatorio-injecao') {
      if (!podeUsarItem(req, 'importar-documentos')) { negarAcesso(res, 'importar documentos'); return true; }
      let body = '';
