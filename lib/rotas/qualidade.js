...
// abaixo" na cadeia). A LEITURA continua livre (modelo novo, ver
// lib/perfis.js: toda página é aberta pra visualização); as rotas de
// ESCRITA exigem um perfil com a área 'qualidade' de edição (Assistente de
// Qualidade, Encarregado, Supervisão, Administrador).

const logger = require('../logger');

module.exports = function criarRotasQualidade({ db, lerOperacoesNaoAvaliadas, removerDaFilaNaoAvaliadas, podeEditarArea, negarEdicao }) {
  ...
  return function tentar(req, res, urlPath) {
      ...
      try {
        ...
        const rows = db.prepare(`
          SELECT id, id_bateria, tipo_montagem, data, fim, turno, capacidade,
                 dimensao, bercos_personalizados
          FROM operacoes
        ...
        //
        // bercosVisuaisPorOperacoes junto: mapa { id_operacao: [{berco,
        // ordem, estado_esquerda, estado_direita}, ...] } — usado pelo
        // Setor de Qualidade pra excluir da grade os painéis cujo lado do
        // berço foi marcado "não enchido" ao vivo em "Bateria Atual" (ver
        // POST /marcar-berco-andamento e _paineisNaoEnchidosDaOperacao,
        // setor-qualidade.js). Nem toda operação da fila necessariamente
        // tem uma linha aqui ainda (corrida rara entre registrar e a
        // linha de bercos_visuais ser criada) — front trata ausência como
        // "nenhum berço marcado", igual sempre tratou antes desta mudança.
        const bercosPorOperacao = db.bercosVisuaisPorOperacoes(ids);
        const lista = rows.map(r => ({
          ...r,
          bercos_personalizados: r.bercos_personalizados ? JSON.parse(r.bercos_personalizados) : null,
          bercos_visuais: bercosPorOperacao[r.id] || null,
        }));
        res.writeHead(200, { 'Content-Type': 'application/json' });
    ...
    if (req.method === 'POST' && urlPath === '/marcar-operacao-avaliada') {
      if (!podeEditarArea(req, 'qualidade')) { negarEdicao(res, 'o Setor de Qualidade'); return true; }
      let body = '';
    ...
    // permissão de controlar operação, de propósito — mesmo nível de
    // fricção baixa de /marcar-berco-andamento e outras rotas internas
    // do dia a dia.
    if (req.method === 'POST' && urlPath === '/registrar-avaliacao-qualidade') {
      ...
      req.on('end', () => {
        try {
          ...
          // ── BLOQUEIO DE TEMPO DE PEGA INVÁLIDO (2ª camada de prevenção) ──
          // Tempo de Pega = Desmoldagem − Enchimento (ver calculateCureTime,
          // setor-qualidade.js). O front já trava isso na tela
          // (_tempoPegaInvalido, chamada em registerEvaluation antes de
          // confirmar), mas a validação de verdade é aqui — quem manda
          // direto pra rota (sem passar pela tela) não consegue burlar.
          // Só considera inválido quando as DUAS datas vieram preenchidas
          // (nem toda avaliação necessariamente tem as duas); diff <= 0
          // (desmoldagem no mesmo instante do enchimento, ou antes dele)
          // também conta como inválido — fisicamente impossível.
          if (avaliacao.dtEnchimento && avaliacao.dtDesmoldagem) {
            const diff = new Date(avaliacao.dtDesmoldagem) - new Date(avaliacao.dtEnchimento);
            if (!Number.isFinite(diff) || diff <= 0) {
              throw new Error('Tempo de Pega inválido — a Data/Hora de Desmoldagem precisa ser posterior à Data/Hora de Enchimento.');
            }
          }

          db.salvarAvaliacaoQualidade(avaliacao);

          // Classificação da operação como avaliada/não avaliada — ANTES,
          // isso só acontecia numa 2ª chamada SEPARADA do front
          // (POST /marcar-operacao-avaliada, feita só depois da resposta
          // desta rota), cuja falha era só logada no console
          // (.catch(err => console.error(...))), nunca reportada ao
          // usuário: a avaliação ficava salva com sucesso (o operador via
          // "Avaliação registrada com sucesso!" e seguia em frente), mas
          // se aquela 2ª chamada falhasse por qualquer motivo (rede,
          // navegação embora antes dela completar, etc.), a operação
          // ficava presa na fila "não avaliada" PRA SEMPRE, sem ninguém
          // perceber (bug real relatado pelo usuário: 2 avaliações que
          // não saíam da fila mesmo já avaliadas). Agora acontece aqui
          // mesmo, na MESMA requisição que salva a avaliação — sem 2º
          // request, sem essa janela de falha silenciosa. Continua num
          // try/catch (só loga, não derruba a resposta) porque a
          // avaliação em si (já salva acima) é o dado que mais importa
          // preservar; uma falha aqui não deveria fingir que o registro
          // inteiro falhou.
          if (avaliacao.linkedOperacaoId) {
            try {
              db.marcarOperacaoAvaliada(avaliacao.linkedOperacaoId); // idempotente
              removerDaFilaNaoAvaliadas(avaliacao.linkedOperacaoId); // idempotente
            } catch (e) {
              logger.error('qualidade', 'Falha ao marcar operação vinculada como avaliada', { erro: e && e.message });
            }
          } else if (avaliacao.batteryId) {
            ...
            catch (e) { logger.error('qualidade', 'Falha ao classificar operação (avaliação avulsa) como avaliada', { erro: e && e.message }); }
          }
    ...
    if (req.method === 'GET' && urlPath === '/avaliacoes-qualidade') {
      try {
        ...
        // Cache-Control explícito (mesmo padrão de contador-tracos.js/
        // sql-admin.js) — esta rota não começa com '/db/', então não pega
        // o monkey-patch global de no-store (ver server.js); e agora que
        // o Debriefing também consome esta MESMA rota (ver debriefing.js
        // — antes usava 'db/avaliacoes_qualidade.json'), vale reforçar
        // aqui também, não só confiar no /db/.
