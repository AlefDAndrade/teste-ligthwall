...
module.exports = function criarRotasOperacaoOffline({
  ...
  function _aprovar(item, validadoPor) {
    ...
    // Data da operação = data do FIM da injeção (mesmo critério do registro
    // ao vivo — ver dataLocal em _registrarOperacaoInterna, operacao.js):
    // uma operação que atravessou a meia-noite tem que aparecer no
    // Debriefing no dia em que foi concluída, não no dia em que começou.
    const dataLocal = (formRecord.fim || formRecord.inicio || '').split('T')[0] || new Date().toISOString().split('T')[0];
