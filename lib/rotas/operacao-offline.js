...
module.exports = function criarRotasOperacaoOffline({
  ...
  // /operacao-offline/corrigir, chamado pela tela de "Operações a
  // Validar" quando alguém clica "🔗 Salvar vínculo") — nenhum dos três
  // (eh_sobra/nota_sobra/link_sobra_original) é uma coluna própria na
  // tabela "tracos" (o INSERT abaixo, em _tracosParaLinhasRelatorio, usa
  // uma lista fechada de campos — não dá pra simplesmente acrescentar
  // mais um). Em vez de mudar o schema só pra isso, tudo é dobrado aqui
  // dentro do próprio campo `obs` — que já É o lugar certo pra uma nota
  // de contexto sobre o traço (mesmo raciocínio de "esta nota é só um
  // guia" que o operador já usa em obs pra outras correções/ajustes).
  function _montarObsComSobra(t) {
    ...
    const vinculo = t.link_sobra_original ? `vinculado a: ${t.link_sobra_original}` : 'ainda sem vínculo com o traço original';
