...
// backup.js: `sessao: sessaoOuAdmin` no
// wiring, server.js).
//
...
// não tem nada a ver com esta aprovação. Por isso a aprovação NUNCA lê o
// snapshot ao vivo compartilhado — mas usa, sim, as marcações que o
// PRÓPRIO dispositivo offline fez em Bateria Atual (ver
// formRecord.bercos_marcados, offline-operacao.js), que são dado próprio
// desta operação, gravado localmente por quem a registrou, sem nenhuma
// relação com o snapshot ao vivo de outra operação (ver _aprovar, abaixo).
...
module.exports = function criarRotasOperacaoOffline({
  ...
  // Registro ao vivo sempre manda paineis_2p/paineis_sp/m2_2p/m2_sp junto
  // com paineis_por_tipo/m2_por_tipo — são "aliases de compatibilidade"
  // calculados por LW.calcPaineis()/calcPaineisPersonalizado() (data.js) e
  // usados hoje pelas colunas fixas "Painéis 2/P"/"Painéis S/P"/"m² 2/P"/
  // "m² S/P" da tabela Registro de Baterias, pela exportação e pelo INSERT
  // (db.operacaoParaRow espera esses 4 campos; sem eles, caem em 0 — ver
  // comentário lá). O calcPaineis local do formulário offline
  // (offline-operacao.js) sempre devolveu paineis_por_tipo/m2_por_tipo
  // certos (por isso o TOTAL de painéis/m² sempre bateu), mas nunca
  // calculou esses 4 aliases — então toda operação offline validada
  // gravava 0 nessas colunas, mesmo tendo painéis do tipo 2p/sp de
  // verdade. Deriva aqui, na aprovação, a partir do paineis_por_tipo/
  // m2_por_tipo que JÁ vêm certos do formulário offline — não depende de
  // nenhuma mudança no dispositivo que registrou, então corrige inclusive
  // itens que já estão parados na fila (aguardando validação) desde antes
  // desta correção.
  function _derivarAliasesTipo(formRecord) {
    const ppt = formRecord.paineis_por_tipo || {};
    const m2pt = formRecord.m2_por_tipo || {};
    return {
      paineis_2p: ppt['2p'] || 0,
      paineis_sp: ppt['sp'] || 0,
      m2_2p: m2pt['2p'] || 0,
  ...
  // Espelha LIMITE_INJECAO_MIN/_diffMinutos de lib/rotas/edicao.js (POST
  // /editar-operacao-avancado) — mesmo raciocínio: tempo_min/houve_atraso
  // SEMPRE recalculados aqui, no servidor, a partir de início/fim/pausas,
  // nunca aceitos prontos do formRecord. formRecord.tempo_min/houve_atraso
  // (o valor que o dispositivo offline calculou sozinho, na hora do
  // registro) é só um palpite do cliente — normalmente correto no registro
  // AO VIVO comum (por isso POST /registrar-operacao confia nele, ver
  // registro-operacao.js), mas aqui, se o Master corrigir início/fim/
  // pausas em POST /operacao-offline/corrigir (o relógio do dispositivo
  // offline pode estar errado — ver README, item 8), o formRecord.tempo_min
  // ficava PARADO no valor de antes da correção, porque
  // atualizarNaFilaOffline (lib/fila-offline.js) só aplica o patch nos
  // campos que vieram nele — nunca recalculava tempo_min/houve_atraso a
  // partir dos novos horários. Recalcular aqui, sempre, na aprovação
  // (em vez de tentar pegar toda vez que /corrigir mexe em inicio/fim/
  // pausas) garante que o valor final gravado em "operacoes" está sempre
  // consistente com os horários que realmente foram aprovados, corrigidos
  // ou não.
  const LIMITE_INJECAO_MIN = 59;
  function _diffMinutos(inicioIso, fimIso) {
    return (new Date(fimIso) - new Date(inicioIso)) / 60000;
  }
  function _recalcularTempoOperacao(formRecord, pausas) {
    const minutosBruto = _diffMinutos(formRecord.inicio, formRecord.fim);
    const minutosPausados = (pausas || []).reduce(
      (acc, p) => acc + (p.pausado_em && p.retomado_em ? _diffMinutos(p.pausado_em, p.retomado_em) : 0), 0
    );
    const tempoMin = minutosBruto - minutosPausados;
    return { tempo_min: tempoMin, houve_atraso: tempoMin > LIMITE_INJECAO_MIN ? 'SIM' : 'NÃO' };
  }
  ...
  // Marcação "Este traço é uma sobra" (offline-operacao.js) + a nota do
  // operador + o vínculo que o Administrador aponta na revisão (ver POST
  // /operacao-offline/corrigir, chamado pelo modal "Vincular Sobra" —
  // cfgAbrirVincularSobra/cfgLinkarSobra, public/js/app-core.js) —
  // nenhum dos três (eh_sobra/nota_sobra/link_sobra_original) é uma
  // coluna própria na tabela "tracos" (o INSERT abaixo, em
  // _tracosParaLinhasRelatorio, usa uma lista fechada de campos — não dá
  // pra simplesmente acrescentar mais um). Em vez de mudar o schema só
  // pra isso, tudo é dobrado aqui dentro do próprio campo `obs` — que já
  // É o lugar certo pra uma nota de contexto sobre o traço (mesmo
  // raciocínio de "esta nota é só um guia" que o operador já usa em obs
  // pra outras correções/ajustes).
  //
  // `link_sobra_original` aceita 2 formatos, mesma dualidade que
  // renderVinculoSobraAtual (app-core.js) já trata do lado da UI:
  //   - objeto estruturado (atual, escolhido no modal "Vincular Sobra" —
  //     calendário + lista de traços do dia, via LW.getRelatorioInjecao):
  //     { id_traco, data, num_traco, id_bateria, berco_inicio, berco_finalizacao }
  //   - string livre (formato antigo, de antes do modal existir — a
  //     pessoa digitava à mão um texto qualquer tipo "operação #123,
  //     traço 4"). Vínculos salvos nesse formato antigo continuam
  //     funcionando (não há migração retroativa dos que já foram
  //     validados — o `obs` final já gravado não muda sozinho).
  function _formatarVinculoSobra(link) {
    if (typeof link === 'string') return link; // formato antigo, texto livre
    const dataFmt = link.data
      ? new Date(link.data + 'T00:00:00').toLocaleDateString('pt-BR')
      : null;
    const partes = [`Traço ${link.num_traco ?? '—'}`];
    if (dataFmt) partes.push(dataFmt);
    if (link.id_bateria) partes.push(`Bateria ${link.id_bateria}`);
    if (link.berco_inicio || link.berco_finalizacao) {
      partes.push(`Berços ${link.berco_inicio ?? '—'}–${link.berco_finalizacao ?? '—'}`);
    }
    return partes.join(' · ');
  }

  function _montarObsComSobra(t) {
    if (!t.eh_sobra) return t.obs || '';
    const nota = t.nota_sobra ? `nota do operador: ${t.nota_sobra}` : 'sem nota do operador';
    const vinculo = t.link_sobra_original
      ? `vinculado a: ${_formatarVinculoSobra(t.link_sobra_original)}`
      : 'ainda sem vínculo com o traço original';
    const marcador = `[♻️ SOBRA — ${nota} — ${vinculo}]`;
    return t.obs ? `${marcador} ${t.obs}` : marcador;
  }
  ...
  function _tracosParaLinhasRelatorio(resumo, tracos, numPorIdTraco) {
    return (tracos || []).map(t => {
      ...
      const obsComSobra = _montarObsComSobra(t); // ver comentário grande acima
      return {
        id_traco: idTraco,
        ultilizado: {
          operacao: [{
        ...
        flow: t.flow_insumo || '',
        obs: obsComSobra, // legado — ver comentário em LW.registrarRelatorioInjecao (data.js)
        silo: t.silo || '',
        expansao: t.expansao || '',
  ...
  function _gravarTracos(linhasTracos) {
    ...
    db.transaction(() => {
      linhasTracos.forEach(novoTraco => {
        ...
        // Insumos CUSTOM (Fase 3, corrigido depois da Fase 6) — roda em
        // QUALQUER caso, mesmo raciocínio de registro-operacao.js (ver
        // comentário lá): INSERT OR IGNORE dentro de
        // salvarInsumosCustomDoTraco já protege contra reescrever um
        // insumo que já tinha valor.
        db.salvarInsumosCustomDoTraco(novoTraco.id_traco, novoTraco.insumos_custom);
  ...
  function _aprovar(item, validadoPor, renumeracao) {
    ...
    const record = {
      ...formRecord,
      // Ver _derivarAliasesTipo, acima — SEMPRE recalculado aqui (nunca
      // "só se ausente"), pra não confiar num paineis_2p/paineis_sp que o
      // cliente porventura já tenha mandado desalinhado do paineis_por_tipo
      // (mesmo formRecord é a fonte da verdade nos dois casos, então o
      // resultado é idêntico quando o cliente já manda os aliases certos).
      ...(_derivarAliasesTipo(formRecord)),
      ...(_recalcularTempoOperacao(formRecord, item.pausas)),
      id: idOperacao,
      data: dataLocal,
    ...
    // Berços Visuais — nasce com as marcações que o PRÓPRIO dispositivo
    // offline fez em Bateria Atual (ver formRecord.bercos_marcados,
    // offline-operacao.js — card local, mesmo formato do snapshot ao vivo
    // usado por criarBercosVisuaisIniciais no registro online). Isso NÃO
    // contradiz o comentário grande do topo do arquivo ("nunca lê/reseta o
    // snapshot ao vivo") — aquele trata do snapshot COMPARTILHADO de uma
    // operação AO VIVO diferente, que pode estar rolando nesta fábrica
    // agora, sem nenhuma relação com esta aprovação; bercos_marcados aqui
    // é dado próprio desta operação offline, gravado pelo mesmo aparelho
    // que a registrou, então não tem esse risco de vazar/misturar dado de
    // operação errada.
    const qtdBercos = parseInt(record.capacidade) || 0;
    db.criarBercosVisuaisIniciais(record.id, qtdBercos, formRecord.bercos_marcados || {});
  ...
  return function tentar(req, res, urlPath, queryParams) {
    ...
    if (req.method === 'POST' && urlPath === '/operacao-offline/enviar') {
      ...
      req.on('end', () => {
        ...
        try {
          const ip = require('../ip-cliente.js').ipRealDoRequest(req) || 'desconhecido';
