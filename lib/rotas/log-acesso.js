    ...
    if (req.method === 'POST' && urlPath === '/registrar-acesso') {
      ...
      req.on('end', () => {
        try {
          ...
          const ip = require('../ip-cliente.js').ipRealDoRequest(req);
          ...
          let acessos = [];
          // Arquivo pode não existir ainda (primeiro acesso registrado) ou
          // estar corrompido — log é best-effort, então começa do zero em
          // vez de derrubar a requisição por causa disso.
