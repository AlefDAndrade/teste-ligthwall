...
// "Agora" em Brasília, como ISO string "fake-UTC" (mesma convenção do
// frontend — ver nowBrasilia()/formatDateTime, public/js/data.js:
// os componentes numéricos JÁ são a hora de Brasília, só com sufixo "Z"
// pra sobreviver a JSON.parse/new Date() sem o navegador reconverter).
// Use esta função em vez de `new Date().toISOString()` (UTC de verdade)
// pra QUALQUER timestamp que vá ser exibido via LW.formatDateTime/
// formatTime — bug recorrente já visto algumas vezes: `new
// Date().toISOString()` no servidor gera UTC de verdade, mas
// formatDateTime assume a convenção fake-UTC-como-Brasília e não
// reconverte, então o horário exibido fica adiantado em 3h (Brasília é
// UTC-3, sem horário de verão desde 2019). Corrigido pela 1ª vez no
// "Quando" do painel de ajustes do Relatório de Injeção
// (registrado_em, ver lib/rotas/leitura-e-ajustes.js).
function agoraBrasiliaISOServer() {
  const fmt = new Intl.DateTimeFormat('en-CA', {
    timeZone: 'America/Sao_Paulo',
    year: 'numeric', month: '2-digit', day: '2-digit',
