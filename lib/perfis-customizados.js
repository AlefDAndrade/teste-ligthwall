...
module.exports = function criarPerfisCustomizados({ fs, path, PRIVATE_DIR, perfis, itensPermissao }) {
  ...
  // Delega pro helper compartilhado (ver lib/itens-permissao.js,
  // validarMapaDePermissoes) — usado também por lib/perfis-fixos-overrides.js,
  // mesma validação, mesma regra de "não mencionado = ocultar".
  function validarPermissoes(permissoes) {
    return itensPermissao.validarMapaDePermissoes(permissoes);
  }
  ...
  function paginasPermitidas(perfilCustomizado) {
    const doCatalogo = itensPermissao.CATALOGO
      .filter(item => nivelDoItem(perfilCustomizado, item.id) !== 'ocultar')
      .map(item => item.id);
    // 'menu' (Menu Principal) NÃO está no catálogo de propósito — não é
    // um item que o Administrador escolhe mostrar/ocultar ao criar um
    // perfil (mesma regra dos 6 perfis fixos, ver PAGINAS_DE_TRABALHO,
    // lib/perfis.js: 'menu' está sempre lá, incondicional). Sem isso,
    // TODO perfil customizado ficava sem 'menu' na lista — e como o
    // Menu é a página de pouso padrão depois do login pra quem não é
    // Operador de Injetora, a pessoa via a tela em branco (showPage
    // silenciosamente recusa navegar pra uma página fora da lista).
    return ['menu', ...doCatalogo];
  }
